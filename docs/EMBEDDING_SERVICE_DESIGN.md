...
## 目标
将 pi-session-manager 扩展为集中式 embedding 服务，供多个 pi 进程共享，避免每个进程独立加载 400MB+ 模型。
...
### 方案 A: 纯 Rust (candle/llm.rs)
**优点:** 单进程，无外部依赖  
**缺点:** GGUF 支持不完善，实现复杂

### 方案 B: Node.js 子进程 (推荐)
**优点:** 复用成熟的 node-llama-cpp，实现快速  
**缺点:** 多一个进程

### 方案 C: 调用外部 llama-server
**优点:** 最灵活，可独立升级  
**缺点:** 需要用户手动管理

## 推荐实现 (方案 B)

### pi-session-manager 变更
1. 添加 `embedding_service.rs` 模块
2. 在 `http_adapter.rs` 添加 `/v1/embedding` 路由
3. 启动时检查模型文件，按需启动 node-llama-cpp 服务
...
impl EmbeddingService {
    ...
    async fn start(&mut self) -> Result<(), Error> {
        // 启动 node-llama-cpp embedding service
...
### pi-gateway (role-persona) 变更
修改 `memory-vector.ts`，添加 HTTP embedding provider：
...
| 场景 | 当前 (每进程) | 新方案 |
|------|--------------|--------|
| 1 pi 进程 | 435 MB | 435 MB |
| 3 pi 进程 | 1.3 GB | 435 MB (共享) |
| 5 pi 进程 | 2.1 GB | 435 MB (共享) |

## 实施步骤

1. **Phase 1**: 创建独立的 embedding-server.js (Node.js)
2. **Phase 2**: pi-session-manager 集成子进程管理
3. **Phase 3**: pi-gateway 添加 HTTP provider 支持
4. **Phase 4**: 测试多进程共享场景

## 备选: 直接使用 llama.cpp server

如果希望更简单，可以直接使用 llama.cpp 的 server 模式：

```bash
./llama-server -m embeddinggemma-300M-Q8_0.gguf --embedding --port 8080
```

然后 pi-session-manager 只做代理，pi-gateway 直接调用 llama-server。
