...
export interface PsmPluginEventEnvelope<
...
export type PsmPluginEventHandler<
  Name extends string = string,
  Payload = unknown,
> = (event: PsmPluginEventEnvelope<Name, Payload>) => void | Promise<void>;
...
export type PsmPluginCommandScope =
  | "global"
  | "project"
  | "session"
  | "selection";
...
export interface PsmToolRenderProps<
  TData extends PsmToolRenderBaseData = PsmToolResolvedData,
> {
...
export type PsmToolRenderComponent<
  TData extends PsmToolRenderBaseData = PsmToolResolvedData,
> = (props: PsmToolRenderProps<TData>) => unknown;

export type PsmToolMatcher =
  | string
  | RegExp
  | ((toolCall: PsmToolCallContent) => boolean);

export interface PsmToolRendererRegistration<
  TData extends PsmToolRenderBaseData = PsmToolResolvedData,
> {
  id: string;
  name: string;
...
export type PsmAppSidebarViewRenderProps<TData = unknown> =
  PsmAppViewRenderProps<TData>;
...
export interface PsmPluginHostContext {
  ...
  registerCommand(
    command: string | PsmPluginCommandRegistration,
    handler?: PsmPluginCommandHandler,
  ): void;
  registerTool(name: string, tool: PsmPluginToolRegistration): void;
  registerSessionEntryTransformer(
    transformer: PsmSessionEntryTransformerRegistration,
  ): void;
}

export type PsmPluginActivateResult = void | PsmPluginDisposable;
export type PsmPluginActivate = (
  ctx: PsmPluginHostContext,
) => PsmPluginActivateResult | Promise<PsmPluginActivateResult>;
...
export interface PsmSessionsClient {
  ...
  readFileChunk(
    sessionPath: string,
    options?: PsmSessionReadChunkOptions,
  ): Promise<PsmSessionChunk>;
...
export type PsmAgentThinkingLevel =
  | "off"
  | "minimal"
  | "low"
  | "medium"
...
export interface PsmAgentClient {
  createSession(
    params: PsmAgentCreateSessionParams,
  ): Promise<PsmAgentSessionHandle>;
  run(params: PsmAgentRunParams): Promise<PsmAgentRunResult>;
  runStream(
    params: PsmAgentRunParams,
    handlers?: PsmAgentRunStreamHandlers,
  ): Promise<PsmAgentRunResult>;
...
export interface PsmJsonConfigClient {
  read<T = unknown>(
    key: string,
    options?: PsmJsonConfigReadOptions<T>,
  ): Promise<T>;
...
export interface PsmFsClient {
  ...
  read(
    rootId: string,
    path: string,
    options?: PsmFsReadOptions,
  ): Promise<PsmFsReadResult>;
