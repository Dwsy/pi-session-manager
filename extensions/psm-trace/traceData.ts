...
export async function loadSessionEntries(client: PsmCapabilityClient, sessionPath: string): Promise<SessionEntry[]> {
  let offset = 0;
  let content = "";
...
export async function loadTraceAnalytics(
  client: PsmCapabilityClient,
  session: TraceSessionReference,
): Promise<SessionTraceAnalytics> {
  const entries = await loadSessionEntries(client, session.path);

  const headerTimestamp = session.created || entries[0]?.timestamp || "";
  ...
  return {
    session_id: session.id || session.path,
    session_path: session.path,
    cwd: session.cwd || "",
    name: session.name || null,
    created: session.created || headerTimestamp,
    modified: session.modified || headerTimestamp,
