...
fn test_full_text_search_builtin_default() {
    ...
    let response = pi_session_manager::full_text_search("内置默认".to_string(), "all".to_string(), None, None, 0, 20, Some("smart".to_string()), Some("newest".to_string()), Some("all".to_string()), None, None);
