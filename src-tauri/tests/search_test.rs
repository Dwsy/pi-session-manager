...
fn test_empty_query_returns_empty_results() {
    ...
    let sessions = vec![SessionInfo {
        ...
        created: DateTime::parse_from_rfc3339("2025-01-01T00:00:00Z").unwrap().with_timezone(&Utc),
        modified: DateTime::parse_from_rfc3339("2025-01-01T00:00:00Z").unwrap().with_timezone(&Utc),
        message_count: 1,
        first_message: "Hello world".to_string(),
        user_messages_text: "".to_string(),
...
{"type":"message","id":"msg2","timestamp":"2025-01-01T00:01:00Z","message":{"role":"assistant","content":[{"type":"text","text":"Assistant message with keyword"}]}}"#;
    ...
    assert!(!results[0].matches.is_empty(), "Should have at least 1 match");
...
fn test_multiple_sessions() {
    ...
    let sessions = vec![
        ...
        SessionInfo {
            ...
            created: DateTime::parse_from_rfc3339("2025-01-01T01:00:00Z").unwrap().with_timezone(&Utc),
            modified: DateTime::parse_from_rfc3339("2025-01-01T01:00:00Z").unwrap().with_timezone(&Utc),
            message_count: 1,
        ...
        SessionInfo {
            ...
            created: DateTime::parse_from_rfc3339("2025-01-01T02:00:00Z").unwrap().with_timezone(&Utc),
            modified: DateTime::parse_from_rfc3339("2025-01-01T02:00:00Z").unwrap().with_timezone(&Utc),
            message_count: 1,
...
fn test_special_characters() {
    ...
    // Symbol-only queries are normalized away by the current search pipeline.
    let results = search_sessions(&sessions, "@#$", SearchMode::Content, RoleFilter::All, true);
    assert_eq!(results.len(), 0, "Symbol-only query should not produce a match");
...
fn test_unicode_search() {
    ...
    let session_content = r#"{"type":"message","id":"msg2","timestamp":"2025-01-01T00:00:00Z","message":{"role":"user","content":[{"type":"text","text":"这里的默认识别系统语言是中文界面"}]}}"#;
    let session_path = create_test_session_file(&test_dir, "session2.jsonl", session_content);

    let sessions = vec![SessionInfo {
        path: session_path.clone(),
        id: "session2".to_string(),
        cwd: "/test".to_string(),
        name: Some("中文扩展会话".to_string()),
        created: DateTime::parse_from_rfc3339("2025-01-01T00:00:00Z").unwrap().with_timezone(&Utc),
        modified: DateTime::parse_from_rfc3339("2025-01-01T00:00:00Z").unwrap().with_timezone(&Utc),
        message_count: 1,
        first_message: "这里的默认识别系统语言是中文界面".to_string(),
        user_messages_text: "".to_string(),
        assistant_messages_text: String::new(),
    ...
    assert_eq!(results.len(), 1, "Should find session with normalized Chinese query");
...
fn test_quoted_phrase_content_search_requires_contiguous_match() {
    ...
    let sessions = vec![
        ...
        SessionInfo {
            ...
            created: DateTime::parse_from_rfc3339("2025-01-01T00:00:01Z").unwrap().with_timezone(&Utc),
            modified: DateTime::parse_from_rfc3339("2025-01-01T00:00:01Z").unwrap().with_timezone(&Utc),
            message_count: 1,
    ...
    let results = search_sessions(&sessions, "\"foo bar\"", SearchMode::Content, RoleFilter::All, true);
...
fn test_quoted_phrase_name_search_requires_phrase_and_remainder_terms() {
    ...
    let results = search_sessions(&sessions, "\"foo bar\" baz", SearchMode::Name, RoleFilter::All, true);
