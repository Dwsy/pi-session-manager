...
mod tests {
    ...
    fn write_legacy_settings_db(path: &Path, rows: &[(&str, Value)]) {
        if let Some(parent) = path.parent() {
        ...
        for (key, value) in rows {
            conn.execute(
                ...
                params![
                    key,
                    serde_json::to_string(value).expect("serialize db value")
                ],
            )
    ...
    #[test]
    fn migrates_legacy_sources_when_unified_config_missing() {
        let _guard = lock_env();
        ...
        let legacy_config_dir = home.join("legacy-config-dir");
        std::env::set_var("PPM_TEST_CONFIG_DIR", &legacy_config_dir);

        fs::write(
            agent_dir.join("session-manager-config.toml"),
            concat!(
                "realtime_cutoff_days = 2\n",
                "scan_interval_seconds = 77\n",
                "enable_fts5 = false\n",
                "preload_count = 20\n",
        ...
        let legacy_db = home.join("legacy-settings.db");
        std::env::set_var("PPM_TEST_DB", &legacy_db);
        write_json(
            &legacy_config_dir.join("pi-session-manager.json"),
            &json!({
                "ws_enabled": false,
                "ws_port": 5001,
                "http_enabled": true,
        ...
        write_legacy_settings_db(
            ...
            &[
                ...
                (
                    "app_settings",
                    json!({
                        "search": { "includeThinkingInSearch": true },
                        "appearance": { "density": "compact" }
                    }),
                ),
                ("session_paths", json!(["/tmp/extra-sessions"])),
                ("window_zoom_level", json!(1.25)),
            ],
        );

        let root = load_root().expect("load migrated root");
        let config_path = config_file_path().expect("config path");
        assert!(config_path.exists(), "new config.json should be created");

        assert_eq!(root["meta"]["schemaVersion"], json!(1));
        assert!(root["meta"]["migratedAt"].is_string());
        assert!(root["meta"]["migrationSources"].is_array());
        assert_eq!(root["server"]["ws_enabled"], json!(false));
        assert_eq!(root["server"]["http_port"], json!(5002));
        assert_eq!(root["server"]["bind_addr"], json!("0.0.0.0"));
        let session_config = crate::config::load_config().expect("load migrated session config");
        assert_eq!(session_config.scan_interval_seconds, 77);
        assert!(!session_config.enable_fts5);
        assert_eq!(
            session_config.session_paths,
            vec!["/tmp/extra-sessions".to_string()]
        );
        assert_eq!(
            root["app"]["search"]["includeThinkingInSearch"],
            json!(true)
        );
        assert_eq!(root["ui"]["windowZoomLevel"], json!(1.25));
        assert!(root.get("models").is_none());
        assert!(root.get("pi").is_none());

        std::env::remove_var("PPM_TEST_DB");
        std::env::remove_var("PPM_TEST_CONFIG_DIR");
        std::env::remove_var("HOME");
    }

    #[test]
    fn existing_unified_config_prevents_repeat_legacy_migration() {
        let _guard = lock_env();
        std::env::remove_var("HOME");
        std::env::remove_var("PPM_TEST_DB");
        ...
        write_json(
            ...
            &json!({
                ...
                "session": {
                    ...
                    "sessionPaths": ["/already-migrated"]
                },
                "app": {
                    "search": { "includeThinkingInSearch": false }
                },
                "models": {
                    "providers": {
                        "stable": {
        ...
        write_json(
            ...
            &json!({
                "providers": {
                    "legacy": { "models": [{ "id": "should-not-load" }] }
                }
            }),
        );
        write_json(
            &agent_dir.join("settings.json"),
            &json!({ "theme": "legacy-theme" }),
        );
        ...
        write_legacy_settings_db(
            ...
            &[
                ...
                ("window_zoom_level", json!(1.75)),
            ],
        );

        let first = load_root().expect("load existing config");
        assert_eq!(first["meta"]["schemaVersion"], json!(1));
        assert_eq!(first["meta"]["migratedAt"], Value::Null);
        assert_eq!(first["server"]["bind_addr"], json!("127.0.0.9"));
        assert!(first.get("models").is_none());
        assert!(first.get("pi").is_none());

        write_json(
            &agent_dir.join("models.json"),
            &json!({
                "providers": {
                    "legacy": { "models": [{ "id": "changed-again" }] }
                }
            }),
        );
        write_legacy_settings_db(
            ...
            &[(
                "server_settings",
                json!({ "bind_addr": "10.0.0.1", "http_port": 1234 }),
            )],
        );

        let second = load_root().expect("reload config");
        assert_eq!(second["server"]["bind_addr"], json!("127.0.0.9"));
        assert_eq!(second["server"]["http_port"], json!(52131));
        assert!(second.get("models").is_none());
        assert!(second.get("pi").is_none());
        assert_eq!(second["ui"]["windowZoomLevel"], json!(1.0));
