...
pub fn gzip_compress(data: &[u8]) -> Result<Vec<u8>, String> {
    ...
    encoder.read_to_end(&mut compressed).map_err(|e| format!("Gzip compression failed: {e}"))?;
    Ok(compressed)
}
...
fn base64_decode(s: &str) -> Result<Vec<u8>, String> {
    use base64::Engine;
    base64::engine::general_purpose::STANDARD.decode(s).map_err(|e| format!("Base64 decode failed: {e}"))
}
