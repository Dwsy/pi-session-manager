...
pub fn parse_meta_json(content: &str) -> Option<SubagentRunInfo> {
    ...
    Some(SubagentRunInfo {
        run_id: v.get("runId").and_then(|x| x.as_str()).unwrap_or("unknown").to_string(),
        agent: v.get("agent").and_then(|x| x.as_str()).unwrap_or("unknown").to_string(),
        model: v.get("model").and_then(|x| x.as_str()).unwrap_or("unknown").to_string(),
        ...
        cache_write_tokens: usage.get("cacheWrite").and_then(|x| x.as_u64()).unwrap_or(0) as usize,
...
pub fn aggregate_runs(runs: &[SubagentRunInfo]) -> SubagentSummary {
    ...
    for run in runs {
        ...
        let agent_stats = runs_by_agent.entry(run.agent.clone()).or_insert(AgentStats { runs: 0, cost: 0.0, tokens: 0 });
        agent_stats.runs += 1;
    ...
    SubagentSummary { total_cost, total_runs: runs.len(), total_tokens, runs_by_agent, runs_by_model }
}
...
pub fn scan_subagent_artifacts(session_dirs: &[PathBuf], conn: Option<&rusqlite::Connection>) -> SubagentSummary {
    ...
    for dir in session_dirs {
        ...
        let entries = match fs::read_dir(&artifacts_dir) {
            Ok(e) => e,
            Err(err) => {
                warn!("Failed to read subagent-artifacts dir {:?}: {}", artifacts_dir, err);
                continue;
            }
        };
        ...
        for entry in entries.flatten() {
            ...
            let file_mtime: Option<DateTime<Utc>> = fs::metadata(&path).ok().and_then(|m| m.modified().ok()).map(|t| {
                let dur = t.duration_since(std::time::UNIX_EPOCH).unwrap_or_default();
                Utc.timestamp_opt(dur.as_secs() as i64, 0).single().unwrap_or_else(Utc::now)
            });
            ...
            if let (Some(mtime), Some(c)) = (file_mtime, conn) {
                if let Ok(Some((cached_mtime, run_info))) = crate::data::sqlite::get_cached_subagent_meta(c, &path_str) {
            ...
            match parse_meta_json(&content) {
                Some(run_info) => {
                    ...
                    if let (Some(mtime), Some(c)) = (file_mtime, conn) {
                        if let Err(err) = crate::data::sqlite::upsert_subagent_meta(c, &path_str, mtime, &run_info) {
...
mod tests {
    ...
    fn aggregate_runs_totals() {
        let runs = vec![
            SubagentRunInfo { run_id: "a".to_string(), agent: "worker".to_string(), model: "opus".to_string(), exit_code: 0, cost: 0.10, input_tokens: 100, output_tokens: 200, cache_read_tokens: 0, cache_write_tokens: 0, duration_ms: 1000, tool_count: 5, timestamp: 0, turns: 3 },
            SubagentRunInfo { run_id: "b".to_string(), agent: "scout".to_string(), model: "haiku".to_string(), exit_code: 0, cost: 0.05, input_tokens: 50, output_tokens: 75, cache_read_tokens: 0, cache_write_tokens: 0, duration_ms: 500, tool_count: 2, timestamp: 0, turns: 2 },
            SubagentRunInfo { run_id: "c".to_string(), agent: "worker".to_string(), model: "opus".to_string(), exit_code: 0, cost: 0.20, input_tokens: 200, output_tokens: 300, cache_read_tokens: 0, cache_write_tokens: 0, duration_ms: 2000, tool_count: 8, timestamp: 0, turns: 5 },
        ];
