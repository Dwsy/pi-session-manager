...
async fn dispatch_json(
    ...
    dispatch(app_state, command, &payload).await
}
...
pub(crate) async fn v1_full_text_search(
    ...
    match readonly::full_text_search(&dispatch_fn, req, true).await {
...
pub(crate) async fn v1_memory_recall(
    ...
    match readonly::memory_recall(&dispatch_fn, req).await {
        Ok(result) => {
...
pub(crate) async fn v1_experience_extract(
    ...
    match readonly::experience_extract(&dispatch_fn, req, 8).await {
...
pub(crate) async fn v1_workflow_route_suggest(
    ...
    match readonly::workflow_route_suggest(&dispatch_fn, req).await {
        Ok(result) => json_success_response(json!({
            "query": result.query,
            "intent": result.intent,
            "confidence": result.confidence,
...
pub(crate) async fn v1_memory_unified(
    ...
    match readonly::memory_unified(&dispatch_fn, req, 6).await {
        Ok(result) => json_success_response(json!({
            "query": result.query,
            "intent": result.intent,
            "confidence": result.confidence,
...
pub(crate) async fn v1_observability_summary(
    ...
    match readonly::analytics_overview() {
        Ok(overview) => json_success_response(json!({
            ...
            "mode": "readonly",
            "capabilities": readonly::readonly_capabilities(false, false),
            "endpoints": [
                "/v1/sessions",
                "/v1/memory/recall",
                "/v1/memory/unified",
