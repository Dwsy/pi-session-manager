...
pub fn read_canonical_session_from_path(path: &Path) -> Result<(ProviderKind, CanonicalSession), String> {
...
pub fn read_canonical_session_from_str(content: &str, path_hint: Option<&Path>) -> Result<(ProviderKind, CanonicalSession), String> {
...
pub fn preview_session_format(canonical: &CanonicalSession, target: ProviderKind) -> Result<String, String> {
...
pub fn convert_canonical_session(source: ProviderKind, canonical: &CanonicalSession, target: ProviderKind, dry_run: bool, force: bool) -> Result<ConversionOutcome, String> {
    if source == target {
        let resume_command = target.resume_command(&canonical.session_id, &canonical.source_path);
        return Ok(ConversionOutcome { target_session_id: canonical.session_id.clone(), written_paths: vec![canonical.source_path.clone()], resume_command, dry_run, warnings: vec!["Source and target provider are the same; skipped writing.".to_string()] });
    }
    ...
    if dry_run {
        return Ok(ConversionOutcome { target_session_id: plan.target_session_id, written_paths: vec![plan.target_path], resume_command: plan.resume_command, dry_run: true, warnings: vec!["Dry run only; no files were written.".to_string()] });
    }
    ...
    let written_path = if target == ProviderKind::OpenCode {
        ...
        let outcome = write_support::atomic_write(&plan.target_path, rendered.as_bytes(), force, target.display_name())?;

        if let Err(error) = write_support::verify_written_session(canonical, target, &outcome.target_path) {
    ...
    Ok(ConversionOutcome { target_session_id: plan.target_session_id, written_paths: vec![written_path], resume_command: plan.resume_command, dry_run: false, warnings: Vec::new() })
}
...
pub fn backing_file_path(path: &Path) -> PathBuf {
    detect_provider(Some(path), "").map(|provider| provider.backing_store_path(path)).unwrap_or_else(|| path.to_path_buf())
}

fn file_modified_time(path: &Path) -> Result<DateTime<Utc>, String> {
    fs::metadata(backing_file_path(path)).and_then(|m| m.modified()).map(DateTime::<Utc>::from).map_err(|e| format!("Failed to get modified time: {e}"))
}
