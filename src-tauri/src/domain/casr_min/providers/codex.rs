...
"#;
        ...
        assert!(session.messages.iter().any(|message| { message.role == MessageRole::Assistant && message.tool_calls.iter().any(|call| call.name == "exec_command" && call.id.as_deref() == Some("call_legacy")) }));
