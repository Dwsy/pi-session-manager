...
pub fn extract_trace_analytics(session_path: &str) -> Result<SessionTraceAnalytics, String> {
    ...
    for line in &lines[1..] {
        if let Ok(value) = serde_json::from_str::<Value>(line) {
            ...
            raw_entries.push(RawEntry { value, timestamp_ms: ts_ms });
        }
    }
    ...
    for (i, raw) in raw_entries.iter().enumerate() {
        ...
        let next_ts = if i + 1 < raw_entries.len() { raw_entries[i + 1].timestamp_ms } else { ts_ms };
        ...
        match entry_type_str {
            "message" => {
                ...
                match role.as_str() {
                    ...
                    "assistant" => {
                        ...
                        // Model tracking
                        let model_key = if let (Some(p), Some(m)) = (&provider, &model) { format!("{p}/{m}") } else { model.clone().unwrap_or_else(|| "unknown".to_string()) };
                        *model_counts.entry(model_key.clone()).or_insert(0) += 1;
                        ...
                        if let Some(ref t) = tokens {
                            ...
                            let mt = tokens_by_model.entry(model_key.clone()).or_insert(TraceTokens { input: 0, output: 0, cache_read: 0, cache_write: 0, total: 0 });
                            mt.input += t.input;
                            mt.output += t.output;
                        ...
                        if let Some(ref c) = cost {
                            ...
                            let mc = cost_by_model.entry(model_key.clone()).or_insert(TraceCost { input: 0.0, output: 0.0, cache_read: 0.0, cache_write: 0.0, total: 0.0 });
                            mc.input += c.input;
                            mc.output += c.output;
                        ...
                        // Thinking
                        let thinking = msg.get("thinking").and_then(|v| v.as_str()).map(|s| truncate(s, CONTENT_PREVIEW_MAX));
                    ...
                    "toolResult" => {
                        ...
                        let error_message = if is_error { content_preview.clone() } else { None };
            ...
            "compaction" => {
                ...
                events.push(TraceEvent {
                    ...
                    model: None,
                    provider: None,
                    thinking: raw.value.get("summary").and_then(|v| v.as_str()).map(|s| truncate(s, CONTENT_PREVIEW_MAX)),
                    tool_calls: vec![],
                    tokens: None,
                    cost: None,
                    content_preview: raw.value.get("summary").and_then(|v| v.as_str()).map(|s| truncate(s, CONTENT_PREVIEW_MAX)),
                    is_error: false,
                    error_message: None,
                    files_read: vec![],
                    files_written: vec![],
            ...
            "custom_message" => {
                events.push(TraceEvent {
                    ...
                    tokens: None,
                    cost: None,
                    content_preview: raw.value.get("content").and_then(|v| v.as_str()).map(|s| truncate(s, CONTENT_PREVIEW_MAX)),
                    is_error: false,
                    error_message: None,
                    files_read: vec![],
                    files_written: vec![],
    ...
    let primary_model = model_counts.iter().max_by_key(|(_, &count)| count).map(|(model, _)| model.clone()).unwrap_or_else(|| "unknown".to_string());

    let mut models_used: Vec<String> = model_counts.keys().cloned().collect();
    models_used.sort();

    let duration_secs = if total_raw > 0 { (raw_entries[total_raw - 1].timestamp_ms.saturating_sub(header_ts)) / 1000 } else { 0 };
    ...
    let bash_commands: Vec<BashCommandStat> = bash_cmd_counts.into_iter().map(|(cmd, count)| BashCommandStat { command_prefix: cmd, count }).collect();
    ...
    Ok(SessionTraceAnalytics {
        ...
        modified: raw_entries.last().map(|r| r.value["timestamp"].as_str().unwrap_or("").to_string()).unwrap_or_else(|| header_ts_str.to_string()),
        duration_secs,
        active_secs,
...
fn extract_usage(msg: &Value) -> (Option<TraceTokens>, Option<TraceCost>) {
    ...
    let tokens = TraceTokens { input: usage["input"].as_u64().unwrap_or(0), output: usage["output"].as_u64().unwrap_or(0), cache_read: usage["cacheRead"].as_u64().unwrap_or(0), cache_write: usage["cacheWrite"].as_u64().unwrap_or(0), total: usage["totalTokens"].as_u64().unwrap_or(0) };

    let cost_val = usage.get("cost");
    let cost = if let Some(c) = cost_val {
        TraceCost { input: c["input"].as_f64().unwrap_or(0.0), output: c["output"].as_f64().unwrap_or(0.0), cache_read: c["cacheRead"].as_f64().unwrap_or(0.0), cache_write: c["cacheWrite"].as_f64().unwrap_or(0.0), total: c["total"].as_f64().unwrap_or(0.0) }
    } else {
        TraceCost { input: 0.0, output: 0.0, cache_read: 0.0, cache_write: 0.0, total: 0.0 }
    };
...
fn extract_tool_calls(msg: &Value) -> Vec<TraceToolCall> {
    ...
    content
        ...
        .map(|block| {
            ...
            let args_preview = args_raw.as_deref().map(|s| truncate(s, ARGS_PREVIEW_MAX)).unwrap_or_default();

            TraceToolCall { id, name, arguments_preview: args_preview, arguments_raw: args_raw, status: "running".to_string(), result_preview: None }
        })
        .collect()
}
