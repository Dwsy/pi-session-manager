...
fn build_masked_curl_command(url: &str, headers: &HeaderMap, body: &str) -> String {
    ...
    for (name, value) in headers {
        ...
        let display_value = if key == "authorization" || key == "x-api-key" { "***" } else { value_text };
        parts.push(format!("-H '{}: {}'", shell_escape_single_quotes(name.as_str()), shell_escape_single_quotes(display_value)));
    }
...
fn parse_headers_json(provider_headers: Option<&Map<String, Value>>, model_headers: Option<&Map<String, Value>>) -> Result<HeaderMap, String> {
    ...
    for raw_map in [provider_headers, model_headers].into_iter().flatten() {
        for (k, v) in raw_map {
            ...
            let header_name = HeaderName::from_bytes(k.as_bytes()).map_err(|e| format!("Invalid header name `{k}`: {e}"))?;
            let header_value = HeaderValue::from_str(&resolved_value).map_err(|e| format!("Invalid header value for `{k}`: {e}"))?;
...
fn build_anthropic_messages_attempts(base_url: &str, model: &str, prompt_text: &str) -> Vec<HttpTestAttempt> {
    let version_header = (HeaderName::from_static("anthropic-version"), HeaderValue::from_static("2023-06-01"));
...
pub fn extract_response_preview(api: &str, response_text: &str) -> Option<String> {
    ...
    match api {
        "openai-completions" => {
            parsed.get("choices").and_then(|v| v.as_array()).and_then(|choices| choices.first()).and_then(|choice| choice.get("message")).and_then(|message| message.get("content")).and_then(|content| content.as_str()).map(str::trim).filter(|value| !value.is_empty()).map(str::to_string)
        }
        "openai-responses" => parsed.get("output_text").and_then(|v| v.as_str()).map(str::trim).filter(|value| !value.is_empty()).map(str::to_string).or_else(|| {
            parsed
                .get("output")
                .and_then(|v| v.as_array())
                .and_then(|items| items.iter().find_map(|item| item.get("content").and_then(|v| v.as_array()).and_then(|content| content.iter().find_map(|part| part.get("text").and_then(|v| v.as_str()).map(str::trim).filter(|value| !value.is_empty()).map(str::to_string)))))
        }),
        "anthropic-messages" => parsed.get("content").and_then(|v| v.as_array()).and_then(|items| items.iter().find_map(|item| item.get("text").and_then(|v| v.as_str()).map(str::trim).filter(|value| !value.is_empty()).map(str::to_string))),
        _ => None,
    }
}
...
fn extract_error_message(response_text: &str) -> Option<String> {
    ...
    parsed
        .get("error")
        .and_then(|value| if let Some(message) = value.get("message").and_then(|message| message.as_str()) { Some(message) } else { value.as_str() })
...
fn extract_provider_and_model<'a>(config: &'a Value, provider: &str, model: &str) -> Result<(&'a Map<String, Value>, Option<&'a Map<String, Value>>), String> {
    let providers = config.get("providers").and_then(|v| v.as_object()).ok_or("Invalid models.json: missing providers object")?;

    let provider_obj = providers.get(provider).and_then(|v| v.as_object()).ok_or_else(|| format!("Provider not found in models.json: {provider}"))?;

    let model_obj = provider_obj.get("models").and_then(|v| v.as_array()).and_then(|models| {
...
pub async fn test_model_http_internal(provider: String, model: String, prompt: Option<String>, timeout_ms: Option<u64>) -> Result<ModelHttpTestResult, String> {
    let config = read_models_config_internal()?;
    let (provider_obj, model_obj) = extract_provider_and_model(&config, &provider, &model)?;

    let base_url = provider_obj.get("baseUrl").and_then(|v| v.as_str()).ok_or_else(|| format!("Provider `{provider}` is missing baseUrl"))?.to_string();

    let api = model_obj.and_then(|obj| obj.get("api").and_then(|v| v.as_str())).or_else(|| provider_obj.get("api").and_then(|v| v.as_str())).unwrap_or("openai-completions").to_string();

    let api_key_raw = provider_obj.get("apiKey").and_then(|v| v.as_str()).unwrap_or("");
    let resolved_api_key = if api_key_raw.is_empty() { String::new() } else { resolve_dynamic_value(api_key_raw)? };
    ...
    base_headers.entry(CONTENT_TYPE).or_insert_with(|| HeaderValue::from_static("application/json"));

    if auth_header && !resolved_api_key.is_empty() {
        let bearer = format!("Bearer {resolved_api_key}");
        let value = HeaderValue::from_str(&bearer).map_err(|e| format!("Invalid authorization header value: {e}"))?;
        base_headers.insert(HeaderName::from_static("authorization"), value);
    }

    if api == "anthropic-messages" && !resolved_api_key.is_empty() {
        let key_value = HeaderValue::from_str(&resolved_api_key).map_err(|e| format!("Invalid x-api-key header value: {e}"))?;
        base_headers.entry(HeaderName::from_static("x-api-key")).or_insert(key_value);
    }

    let prompt_text = prompt.unwrap_or_else(|| "Reply with a concise health-check response that includes the word ok.".to_string());
    ...
    let attempts = match api.as_str() {
        ...
        "google-generative-ai" => return Err("google-generative-ai testing is not supported yet in model HTTP tester".to_string()),
    ...
    let client = reqwest::Client::builder().timeout(timeout).build().map_err(|e| format!("Create HTTP client failed: {e}"))?;
    ...
    for (index, attempt) in attempts.iter().enumerate() {
        ...
        let request_body = serde_json::to_string_pretty(&attempt.body_value).map_err(|e| format!("Serialize HTTP test request body: {e}"))?;
        let curl_command = build_masked_curl_command(&attempt.url, &headers, &request_body);

        let start = Instant::now();
        let response_result = client.post(&attempt.url).headers(headers).json(&attempt.body_value).send().await;
        ...
        let result = match response_result {
            Ok(response) => {
                ...
                let error = if status.is_success() {
                    ...
                    let detail = extract_error_message(&response_text).unwrap_or_else(|| format!("HTTP status {}", status.as_u16()));
        ...
        let should_retry = api == "openai-completions" && !result.ok && index + 1 < attempts.len() && matches!(result.status_code, Some(400) | Some(404) | Some(422) | None);
