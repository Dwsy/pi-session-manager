...
pub fn upsert_scan_state_for_session(conn: &Connection, session: &SessionInfo, file_modified: DateTime<Utc>, last_parse_status: &str) -> Result<(), String> {
...
pub fn delete_scan_state(conn: &Connection, path: &str) -> Result<(), String> {
    conn.execute("DELETE FROM scan_state WHERE path = ?1", params![path]).map_err(|e| format!("Failed to delete scan_state for {path}: {e}"))?;
    Ok(())
}
...
mod tests {
    ...
    fn scan_state_round_trip_works() {
        ...
        let conn = crate::data::sqlite::init_db_with_path(&db_path, &Config::default()).expect("db");
        let now = Utc::now();

        upsert_scan_state(&conn, "/tmp/session.jsonl", "/tmp/session.jsonl", "pi", now, 123, "ok").expect("upsert scan_state");
