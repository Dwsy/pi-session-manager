...
use casr::model::{CanonicalMessage, CanonicalSession, MessageRole, flatten_content, normalize_role, parse_timestamp, truncate_title};
...
fn iso_to_millis(s: &str) -> i64 {
    parse_timestamp(&serde_json::Value::String(s.to_string())).unwrap_or_else(|| panic!("Failed to parse timestamp: {s}"))
}

/// Helper: parse a float epoch-seconds value to millis the same way casr does.
fn float_secs_to_millis(secs: f64) -> i64 {
    parse_timestamp(&serde_json::json!(secs)).unwrap_or_else(|| panic!("Failed to parse float timestamp: {secs}"))
}

/// Helper: parse integer epoch-seconds to millis the same way casr does.
fn int_secs_to_millis(secs: i64) -> i64 {
    parse_timestamp(&serde_json::json!(secs)).unwrap_or_else(|| panic!("Failed to parse int timestamp: {secs}"))
}
...
impl<'a> MessageExpectation<'a> {
    fn assert_matches(&self, msg: &CanonicalMessage, fixture_id: &str) {
        ...
        for needle in self.content_contains {
            assert!(msg.content.contains(needle), "{ctx} content should contain '{needle}' but was: '{}'", &msg.content[..msg.content.len().min(200)]);
        }

        if let Some(expected_ts) = self.timestamp_millis {
            let actual_ts = msg.timestamp.unwrap_or_else(|| panic!("{ctx} expected timestamp {expected_ts} but got None"));
            // Allow 1ms tolerance for float rounding.
            assert!((actual_ts - expected_ts).abs() <= 1, "{ctx} timestamp mismatch: expected {expected_ts}, got {actual_ts}");
        } else {
        ...
        assert_eq!(actual_tc_names, expected_tc, "{ctx} tool_call names mismatch");

        assert_eq!(msg.tool_results.len(), self.tool_result_count, "{ctx} tool_result count mismatch");

        for key in self.extra_keys {
            assert!(msg.extra.get(key).is_some(), "{ctx} expected extra key '{key}' to be present");
        }
    }
}
...
mod cc_parity {
    ...
    fn read_cc(name: &str) -> CanonicalSession {
        ...
        ClaudeCode.read_session(&path).unwrap_or_else(|e| panic!("Failed to read {name}: {e}"))
    }
    ...
    fn cc_simple_deep_content() {
        ...
        assert_eq!(s.workspace.as_deref(), Some(Path::new("/data/projects/myapp")));
        ...
        let expectations = [
            ...
            MessageExpectation {
                ...
                content_exact: Some("I found the issue in auth.rs. The token validation was using an expired key."),
                content_contains: &[],
            ...
            MessageExpectation { idx: 2, role: MessageRole::User, content_exact: Some("Great, can you also add a test for it?"), content_contains: &[], timestamp_millis: ts("2026-01-15T10:01:00.000Z"), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &["type", "uuid"] },
            MessageExpectation {
                idx: 3,
                role: MessageRole::Assistant,
    ...
    fn cc_complex_tool_calls_and_filtering() {
        ...
        assert_eq!(s.workspace.as_deref(), Some(Path::new("/data/projects/webapp")));
        assert_eq!(s.model_name.as_deref(), Some("claude-sonnet-4-5-20250929"));
        ...
        assert_eq!(s.messages[0].content, "Refactor the API handler to use async/await");
    ...
    fn cc_simple_metadata_structure() {
        ...
        let meta = s.metadata.as_object().expect("metadata should be an object");
        // Claude Code reader stores claudeVersion and gitBranch in metadata.
        assert!(meta.contains_key("claudeVersion") || meta.contains_key("gitBranch"), "metadata should contain Claude-specific keys: {meta:?}");
    }
    ...
    fn cc_simple_extra_preserves_native_fields() {
        ...
        for msg in &s.messages {
            ...
            assert!(extra.contains_key("sessionId"), "extra should contain sessionId");
        }
    }
    ...
    fn cc_unicode_content_fidelity() {
        ...
        assert!(s.messages[0].content.contains("\u{3053}\u{3093}\u{306b}\u{3061}\u{306f}"));
        assert!(s.messages[0].content.contains("\u{4f60}\u{597d}\u{4e16}\u{754c}"));
    ...
    fn cc_missing_workspace_returns_none() {
        ...
        assert!(s.workspace.is_none(), "workspace should be None when no cwd");
    }
    ...
    fn cc_entry_type_filtering_parity() {
        ...
        let s = read_cc("cc_complex");
        for msg in &s.messages {
            let entry_type = msg.extra.get("type").and_then(|v| v.as_str()).unwrap_or("unknown");
            assert!(entry_type == "user" || entry_type == "assistant", "Only user/assistant entry types should produce messages, got: {entry_type}");
        }
    }
...
mod codex_parity {
    ...
    fn read_codex(name: &str) -> CanonicalSession {
        let ext = if name == "codex_legacy" { "json" } else { "jsonl" };
        let path = fixtures_dir().join(format!("codex/{name}.{ext}"));
        Codex.read_session(&path).unwrap_or_else(|e| panic!("Failed to read {name}: {e}"))
    }
    ...
    fn codex_modern_deep_content() {
        ...
        assert_eq!(s.workspace.as_deref(), Some(Path::new("/data/projects/backend")));

        let ts = |secs: f64| Some(float_secs_to_millis(secs));

        let expectations = [
            MessageExpectation { idx: 0, role: MessageRole::User, content_exact: Some("Optimize the database query in users.rs"), content_contains: &[], timestamp_millis: ts(1_737_100_001.0), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &[] },
            MessageExpectation { idx: 1, role: MessageRole::Assistant, content_contains: &["index hint", "batching"], content_exact: None, timestamp_millis: ts(1_737_100_010.0), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &[] },
            MessageExpectation { idx: 2, role: MessageRole::User, content_exact: Some("Can you also add connection pooling?"), content_contains: &[], timestamp_millis: ts(1_737_100_020.0), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &[] },
            MessageExpectation { idx: 3, role: MessageRole::Assistant, content_contains: &["connection pool", "r2d2"], content_exact: None, timestamp_millis: ts(1_737_100_030.0), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &[] },
        ];
    ...
    fn codex_legacy_deep_content() {
        ...
        assert_eq!(s.workspace.as_deref(), Some(Path::new("/data/projects/oldproject")));
    ...
    fn codex_reasoning_messages_have_author() {
        ...
        for (msg, expected_author) in s.messages.iter().zip(expected_authors.iter()) {
            assert_eq!(msg.author.as_deref(), *expected_author, "msg[{}] author mismatch", msg.idx);
        }

        // Reasoning messages should have Assistant role.
        let reasoning_msgs: Vec<&CanonicalMessage> = s.messages.iter().filter(|m| m.author.as_deref() == Some("reasoning")).collect();
    ...
    fn codex_modern_timestamp_float_normalization() {
        ...
        let s = read_codex("codex_modern");
        let first_ts = s.messages[0].timestamp.expect("first msg should have timestamp");
    ...
    fn codex_modern_content_flattening_input_text() {
        ...
        assert!(s.messages[1].content.contains("index hint"), "input_text blocks should be flattened correctly");
    }
}
...
mod gemini_parity {
    ...
    fn read_gemini(name: &str) -> CanonicalSession {
        ...
        Gemini.read_session(&path).unwrap_or_else(|e| panic!("Failed to read {name}: {e}"))
    }
    ...
    fn gmi_simple_deep_content() {
        ...
        let expectations = [
            MessageExpectation { idx: 0, role: MessageRole::User, content_exact: Some("Create a REST API endpoint for user profiles"), content_contains: &[], timestamp_millis: ts("2026-01-14T16:00:00.000Z"), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &["type"] },
            MessageExpectation { idx: 1, role: MessageRole::Assistant, content_contains: &["/api/users/:id", "GET", "PUT", "DELETE"], content_exact: None, timestamp_millis: ts("2026-01-14T16:00:30.000Z"), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &["type"] },
            MessageExpectation { idx: 2, role: MessageRole::User, content_exact: Some("Add input validation too"), content_contains: &[], timestamp_millis: ts("2026-01-14T16:02:00.000Z"), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &["type"] },
            MessageExpectation { idx: 3, role: MessageRole::Assistant, content_contains: &["validation", "email", "username"], content_exact: None, timestamp_millis: ts("2026-01-14T16:03:00.000Z"), author: None, tool_call_names: &[], tool_result_count: 0, extra_keys: &["type"] },
        ];
    ...
    fn gmi_grounding_metadata_preserved_in_extra() {
        ...
        let assistant_msgs: Vec<&CanonicalMessage> = s.messages.iter().filter(|m| m.role == MessageRole::Assistant).collect();

        assert_eq!(assistant_msgs.len(), 2);
        for msg in &assistant_msgs {
            assert!(msg.extra.get("groundingMetadata").is_some(), "msg[{}] should have groundingMetadata in extra", msg.idx);
            assert!(msg.extra.get("citations").is_some(), "msg[{}] should have citations in extra", msg.idx);
        }
    }
    ...
    fn gmi_grounding_content_flattens_mixed_blocks() {
        ...
        assert!(first_assistant.content.contains("Rust 2024 edition"), "Text content should be extracted from array blocks");
    }
    ...
    fn gmi_session_level_timestamps() {
        ...
        assert!(s.ended_at.is_some(), "ended_at should be set from timestamps");
        let ended_at = s.ended_at.unwrap();
        assert!(ended_at >= ended, "ended_at should be at least as late as last message");
    }

    #[test]
    fn gmi_missing_workspace_returns_none() {
        let s = read_gemini("gmi_missing_workspace");
        assert!(s.workspace.is_none(), "workspace should be None when no paths in content");
    }
...
mod edge_parity {
    ...
    fn edge_empty_content_messages_skipped() {
        ...
        for msg in &s.messages {
            assert!(!msg.content.trim().is_empty(), "msg[{}] should have non-empty content", msg.idx);
        }
    }
    ...
    fn edge_null_timestamps_all_none() {
        ...
        assert_eq!(s.messages.len(), 4);
        for msg in &s.messages {
            assert!(msg.timestamp.is_none(), "msg[{}] timestamp should be None", msg.idx);
        }
...
mod normalization_parity {
    ...
    fn flatten_mixed_text_and_tool() {
        ...
        assert_eq!(flatten_content(&val), "Let me read the file.\n[Tool: Read - main.rs]");
    }
    ...
    fn parse_timestamp_rfc3339() {
        ...
        assert!(millis > 1_700_000_000_000, "Should be > year 2023 in millis");
        assert!(millis < 1_800_000_000_000, "Should be < year 2027 in millis");
    }
    ...
    fn normalize_role_unknown_becomes_other() {
        assert_eq!(normalize_role("narrator"), MessageRole::Other("narrator".to_string()));
    }
...
mod divergence_tracking {
    ...
    fn divergence_codex_token_count_skipped_not_attached() {
        ...
        for msg in &s.messages {
            assert!(msg.extra.get("cass").is_none(), "casr should not attach CASS-style token_usage to messages");
        }
    }
}
...
mod cross_provider_parity {
    ...
    type FixtureLoaderWithPrefix = (&'static str, Box<dyn Fn() -> CanonicalSession>, &'static str);
    ...
    fn all_fixtures_have_sequential_idx() {
        let fixtures: Vec<FixtureLoader> = vec![
            ("cc_simple", Box::new(|| ClaudeCode.read_session(&fixtures_dir().join("claude_code/cc_simple.jsonl")).unwrap())),
            ("cc_complex", Box::new(|| ClaudeCode.read_session(&fixtures_dir().join("claude_code/cc_complex.jsonl")).unwrap())),
            ("codex_modern", Box::new(|| Codex.read_session(&fixtures_dir().join("codex/codex_modern.jsonl")).unwrap())),
            ("codex_legacy", Box::new(|| Codex.read_session(&fixtures_dir().join("codex/codex_legacy.json")).unwrap())),
            ("gmi_simple", Box::new(|| Gemini.read_session(&fixtures_dir().join("gemini/gmi_simple.json")).unwrap())),
        ];
    ...
    fn all_fixtures_no_empty_content() {
        let fixtures: Vec<FixtureLoader> = vec![
            ("cc_simple", Box::new(|| ClaudeCode.read_session(&fixtures_dir().join("claude_code/cc_simple.jsonl")).unwrap())),
            ("cc_complex", Box::new(|| ClaudeCode.read_session(&fixtures_dir().join("claude_code/cc_complex.jsonl")).unwrap())),
            ("cc_malformed", Box::new(|| ClaudeCode.read_session(&fixtures_dir().join("claude_code/cc_malformed.jsonl")).unwrap())),
            ("codex_modern", Box::new(|| Codex.read_session(&fixtures_dir().join("codex/codex_modern.jsonl")).unwrap())),
            ("codex_reasoning", Box::new(|| Codex.read_session(&fixtures_dir().join("codex/codex_reasoning.jsonl")).unwrap())),
            ("gmi_simple", Box::new(|| Gemini.read_session(&fixtures_dir().join("gemini/gmi_simple.json")).unwrap())),
            ("gmi_grounding", Box::new(|| Gemini.read_session(&fixtures_dir().join("gemini/gmi_grounding.json")).unwrap())),
        ];
        ...
        for (name, loader) in &fixtures {
            ...
            for msg in &s.messages {
                ...
                assert!(has_content || has_tools, "[{name}] msg[{}] should not have completely empty content (needs text or tools)", msg.idx);
            }
        }
    }
    ...
    fn all_fixtures_correct_provider_slug() {
        let cc = ClaudeCode.read_session(&fixtures_dir().join("claude_code/cc_simple.jsonl")).unwrap();
        assert_eq!(cc.provider_slug, "claude-code");

        let cod = Codex.read_session(&fixtures_dir().join("codex/codex_modern.jsonl")).unwrap();
        assert_eq!(cod.provider_slug, "codex");

        let gmi = Gemini.read_session(&fixtures_dir().join("gemini/gmi_simple.json")).unwrap();
    ...
    fn all_fixtures_timestamp_ordering() {
        ...
        for (name, loader) in &fixtures {
            ...
            if let (Some(start), Some(end)) = (s.started_at, s.ended_at) {
                assert!(start <= end, "[{name}] started_at ({start}) should be ≤ ended_at ({end})");
            }
        }
    }
    ...
    fn all_fixtures_message_timestamps_ordered() {
        ...
        for (name, loader) in &fixtures {
            ...
            for window in timestamps.windows(2) {
                assert!(window[0] <= window[1], "[{name}] timestamps should be non-decreasing: {} > {}", window[0], window[1]);
            }
        }
    }
    ...
    fn all_fixtures_title_from_first_user() {
        let fixtures: Vec<FixtureLoaderWithPrefix> = vec![
            ("cc_simple", Box::new(|| ClaudeCode.read_session(&fixtures_dir().join("claude_code/cc_simple.jsonl")).unwrap()), "Fix the login bug"),
            ("codex_modern", Box::new(|| Codex.read_session(&fixtures_dir().join("codex/codex_modern.jsonl")).unwrap()), "Optimize the database query"),
            ("gmi_simple", Box::new(|| Gemini.read_session(&fixtures_dir().join("gemini/gmi_simple.json")).unwrap()), "Create a REST API endpoint"),
        ];

        for (name, loader, prefix) in &fixtures {
            let s = loader();
            let title = s.title.as_deref().unwrap_or("");
            assert!(title.starts_with(prefix), "[{name}] title should start with '{prefix}' but was '{title}'");
        }
    }
}
