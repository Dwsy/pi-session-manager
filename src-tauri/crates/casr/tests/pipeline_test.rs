...
impl MockProvider {
    fn new(name: &str, slug: &str, alias: &str, roots: Vec<PathBuf>) -> Self {
        let state = MockState { installed: true, ..MockState::default() };
        Self { name: name.to_string(), slug: slug.to_string(), alias: alias.to_string(), roots, state: Arc::new(Mutex::new(state)) }
    }

    fn set_owned_session(&self, session_id: &str, path: impl Into<PathBuf>) {
        self.state.lock().expect("mock state lock").owns_by_session_id.insert(session_id.to_string(), path.into());
    }
    ...
    fn set_read_session(&self, path: impl Into<PathBuf>, session: CanonicalSession) {
        self.state.lock().expect("mock state lock").read_by_path.insert(path.into(), ReadOutcome::Session(Box::new(session)));
    }

    fn set_read_error(&self, path: impl Into<PathBuf>, message: &str) {
        self.state.lock().expect("mock state lock").read_by_path.insert(path.into(), ReadOutcome::Error(message.to_string()));
    }

    fn set_write_success(&self, written: WrittenSession) {
        self.state.lock().expect("mock state lock").write_outcome = Some(WriteOutcome::Success(written));
    }

    fn set_write_error(&self, message: &str) {
        self.state.lock().expect("mock state lock").write_outcome = Some(WriteOutcome::Error(message.to_string()));
    }
    ...
    fn last_written(&self) -> Option<CanonicalSession> {
        self.state.lock().expect("mock state lock").last_written.clone()
    }
}
...
impl Provider for MockProvider {
    ...
    fn detect(&self) -> DetectionResult {
        ...
        DetectionResult { installed, version: None, evidence: vec![format!("installed={installed}")] }
    }
    ...
    fn owns_session(&self, session_id: &str) -> Option<PathBuf> {
        self.state.lock().expect("mock state lock").owns_by_session_id.get(session_id).cloned()
    }
    ...
    fn read_session(&self, path: &Path) -> anyhow::Result<CanonicalSession> {
        ...
        Err(anyhow::anyhow!("mock provider '{}' has no read outcome for path {}", self.slug, path.display()))
    }
...
fn msg(idx: usize, role: MessageRole, content: &str, ts: Option<i64>) -> CanonicalMessage {
    CanonicalMessage { idx, role, content: content.to_string(), timestamp: ts, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null }
}
...
fn valid_session_with_id(session_id: &str) -> CanonicalSession {
    CanonicalSession {
        ...
        messages: vec![
            msg(0, MessageRole::User, "question one", Some(1_700_000_000_000)),
            msg(1, MessageRole::Assistant, "answer one", Some(1_700_000_005_000)),
            msg(2, MessageRole::User, "question two", Some(1_700_000_010_000)),
            msg(3, MessageRole::Assistant, "answer two", Some(1_700_000_020_000)),
        ],
...
fn pipeline_convert_happy_path_writes_and_verifies() {
    ...
    let result = pipeline.convert("tgt", "sid-a", options(false, None)).expect("happy path convert should succeed");
...
fn pipeline_dry_run_skips_write() {
    ...
    let result = pipeline.convert("tgt", "sid-b", options(true, None)).expect("dry-run convert should succeed");

    assert!(result.written.is_none(), "dry-run should skip writes");
    assert_eq!(dst.write_calls(), 0, "dry-run should not call write_session");
}

#[test]
fn pipeline_same_provider_short_circuit_skips_write() {
    let provider = MockProvider::new("Mock Source", "mock-source", "src", vec![PathBuf::from("/tmp/src-root")]);
    ...
    let pipeline = ConversionPipeline { registry: ProviderRegistry::new(vec![Box::new(provider.clone())]) };

    let result = pipeline.convert("src", "sid-same", options(false, None)).expect("same-provider conversion should short-circuit");

    assert_eq!(provider.write_calls(), 0, "same-provider conversion should not write");
    ...
    let written = result.written.expect("same-provider should still return resume metadata");
...
fn pipeline_warns_when_target_cli_missing_but_write_succeeds() {
    ...
    let result = pipeline.convert("tgt", "sid-target-cli-missing", options(false, None)).expect("write should still succeed when target detect reports not installed");

    assert!(result.warnings.iter().any(|w| w.contains("not detected as installed")), "expected missing-target warning; got {:?}", result.warnings);
}

#[test]
fn pipeline_unknown_target_alias_errors() {
    let src = MockProvider::new("Mock Source", "mock-source", "src", vec![PathBuf::from("/tmp/src-root")]);
    let pipeline = ConversionPipeline { registry: ProviderRegistry::new(vec![Box::new(src)]) };

    let err = pipeline.convert("missing", "sid-z", options(false, None)).expect_err("unknown target alias should error");
...
fn pipeline_session_not_found_errors() {
    ...
    let err = pipeline.convert("tgt", "missing-session", options(false, None)).expect_err("missing session should error");
...
fn pipeline_ambiguous_session_errors() {
    ...
    let pipeline = ConversionPipeline { registry: ProviderRegistry::new(vec![Box::new(src_a), Box::new(src_b), Box::new(dst)]) };

    let err = pipeline.convert("tgt", "same-id", options(false, None)).expect_err("ambiguous session id should error");

    assert!(matches!(err.downcast_ref::<CasrError>(), Some(CasrError::AmbiguousSessionId { .. })));
}
...
fn pipeline_source_hint_alias_narrows_resolution() {
    ...
    let result = pipeline.convert("tgt", "same-id", options(false, Some("s1".to_string()))).expect("source alias hint should disambiguate");
    assert!(result.written.is_some());
    assert_eq!(dst.last_written().expect("target should capture written session").session_id, "from-a");
}
...
fn pipeline_source_hint_path_bypasses_discovery() {
    ...
    let result = pipeline.convert("tgt", "ignored-by-path-hint", options(false, Some(direct_path.display().to_string()))).expect("path source hint should resolve direct path");

    assert!(result.written.is_some());
    assert_eq!(dst.last_written().expect("target should capture written session").session_id, "direct-session");
}
...
fn pipeline_write_failure_propagates() {
    ...
    let err = pipeline.convert("tgt", "sid-write-fail", options(false, None)).expect_err("write failure should propagate");
    assert!(err.to_string().contains("write failed in mock target"));
    assert_eq!(dst.write_calls(), 1, "write should have been attempted once");
}
...
fn pipeline_readback_mismatch_fails_and_removes_unverified_output() {
    ...
    let pipeline = ConversionPipeline { registry: ProviderRegistry::new(vec![Box::new(src), Box::new(dst)]) };
    let err = pipeline.convert("tgt", "sid-readback-mismatch", options(false, None)).expect_err("readback mismatch should fail conversion");

    match err.downcast_ref::<CasrError>() {
        Some(CasrError::VerifyFailed { detail, .. }) => {
            assert!(detail.contains("message count mismatch"), "unexpected verify detail: {detail}");
        }
        other => panic!("expected VerifyFailed, got {other:?}"),
    }
    assert!(!written_path.exists(), "unverified output should be removed on verify failure");
}
...
fn pipeline_readback_content_mismatch_fails_and_removes_unverified_output() {
    ...
    let pipeline = ConversionPipeline { registry: ProviderRegistry::new(vec![Box::new(src), Box::new(dst)]) };
    let err = pipeline.convert("tgt", "sid-readback-content-mismatch", options(false, None)).expect_err("readback content mismatch should fail conversion");

    match err.downcast_ref::<CasrError>() {
        Some(CasrError::VerifyFailed { detail, .. }) => {
            assert!(detail.contains("content mismatch"), "unexpected verify detail: {detail}");
        }
        other => panic!("expected VerifyFailed, got {other:?}"),
    }
    assert!(!written_path.exists(), "unverified output should be removed on verify failure");
...
fn pipeline_readback_error_restores_backup_and_returns_verify_failed() {
    ...
    let pipeline = ConversionPipeline { registry: ProviderRegistry::new(vec![Box::new(src), Box::new(dst)]) };
    let err = pipeline.convert("tgt", "sid-readback-error", options(false, None)).expect_err("readback error should fail conversion");

    match err.downcast_ref::<CasrError>() {
        Some(CasrError::VerifyFailed { detail, .. }) => {
            assert!(detail.contains("rollback succeeded"), "expected rollback detail, got: {detail}");
        }
        other => panic!("expected VerifyFailed, got {other:?}"),
    }
    ...
    assert!(!backup_path.exists(), "backup should be consumed during restore");
}
...
fn validate_session_errors_for_empty_and_single_sided() {
    ...
    assert!(validate_session(&empty).has_errors(), "empty session should fail validation");

    let mut user_only = valid_session_with_id("user-only");
    user_only.messages.retain(|m| matches!(m.role, MessageRole::User));
    assert!(validate_session(&user_only).has_errors(), "user-only session should fail validation");

    let mut assistant_only = valid_session_with_id("assistant-only");
    assistant_only.messages.retain(|m| matches!(m.role, MessageRole::Assistant));
    assert!(validate_session(&assistant_only).has_errors(), "assistant-only session should fail validation");
}
...
fn validate_session_warnings_and_info_for_quality_issues() {
    ...
    session.messages = vec![msg(0, MessageRole::User, "u1", None), msg(1, MessageRole::User, "u2", None), msg(2, MessageRole::Assistant, "a1", None)];
    session.messages[2].tool_results = vec![ToolResult { call_id: Some("missing-call-id".to_string()), content: "result".to_string(), is_error: false }];
    ...
    assert!(info_joined.contains("unknown tool call id"), "info: {info_joined}");
}

#[test]
fn validate_session_reports_tool_call_info_when_present() {
    let mut session = valid_session_with_id("tool-calls");
    session.messages[1].tool_calls.push(casr::model::ToolCall { id: Some("call-1".to_string()), name: "Read".to_string(), arguments: serde_json::json!({"file":"src/lib.rs"}) });
    let validation = validate_session(&session);
    assert!(validation.info.iter().any(|line| line.contains("Session contains tool calls")), "expected tool-call info line; got {:?}", validation.info);
}
    ...
    S: tracing::Subscriber,
{
    fn on_event(&self, event: &tracing::Event<'_>, _ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        self.events.lock().expect("log collector lock").push(CapturedEvent { level: *meta.level(), fields });
    }
}
...
fn event_has_message(event: &CapturedEvent, needle: &str) -> bool {
    event.fields.get("message").is_some_and(|msg| msg.contains(needle))
}
...
fn pipeline_emits_trace_events_for_detection_read_write_verify() {
    ...
    let subscriber = tracing_subscriber::registry().with(collector.clone().with_filter(tracing_subscriber::filter::LevelFilter::TRACE));
    ...
    assert!(events.iter().any(|e| e.level == tracing::Level::INFO && event_has_message(e, "starting conversion")), "missing starting conversion INFO event; got {events:#?}");
    assert!(events.iter().any(|e| e.level == tracing::Level::TRACE && event_has_message(e, "detection")), "missing provider detection TRACE event; got {events:#?}");
    assert!(events.iter().any(|e| e.level == tracing::Level::DEBUG && event_has_message(e, "found Claude Code session")), "missing session discovery DEBUG event; got {events:#?}");
