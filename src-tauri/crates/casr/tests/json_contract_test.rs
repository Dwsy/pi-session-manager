...
fn setup_cc_fixture(tmp: &TempDir, fixture_name: &str) -> String {
    ...
    let first_line: serde_json::Value = content.lines().find(|l| !l.trim().is_empty()).and_then(|l| serde_json::from_str(l).ok()).expect("fixture should have valid first line");

    let session_id = first_line["sessionId"].as_str().unwrap_or("unknown").to_string();
...
fn setup_gemini_fixture_custom(tmp: &TempDir, fixture_name: &str, workspace_hint: Option<&str>) -> String {
    ...
    fs::write(&target_path, serde_json::to_string_pretty(&root).unwrap()).expect("write Gemini fixture");
...
fn assert_exact_keys(obj: &serde_json::Value, expected: &[&str], ctx: &str) {
    ...
    assert!(extra.is_empty() && missing.is_empty(), "{ctx}: key mismatch.\n  Extra: {extra:?}\n  Missing: {missing:?}\n  Actual keys: {actual:?}");
}
...
fn assert_provider_object(obj: &serde_json::Value, idx: usize) {
    ...
    assert_exact_keys(obj, &["name", "slug", "alias", "installed", "version", "evidence"], &ctx);
...
fn contract_providers_json_shape() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(&stdout).unwrap_or_else(|e| panic!("Invalid JSON from providers: {e}\nOutput: {stdout}"));

    let arr = parsed.as_array().expect("providers --json should be an array");
    assert_eq!(arr.len(), 17, "should list 17 providers (CC, Codex, Gemini, Antigravity, Cursor, Cline, Aider, Amp, OpenCode, ChatGPT, ClawdBot, Vibe, Factory, OpenClaw, Pi-Agent, Kiro, Grok)");
...
fn contract_providers_known_slugs() {
    let tmp = TempDir::new().unwrap();
    let output = casr_cmd(&tmp).args(["--json", "providers"]).output().unwrap();

    let parsed: serde_json::Value = serde_json::from_str(&String::from_utf8_lossy(&output.stdout)).unwrap();
    let slugs: Vec<&str> = parsed.as_array().unwrap().iter().map(|p| p["slug"].as_str().unwrap()).collect();
...
fn contract_providers_aliases_match_slugs() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(&String::from_utf8_lossy(&output.stdout)).unwrap();
    let alias_map: Vec<(&str, &str)> = parsed.as_array().unwrap().iter().map(|p| (p["slug"].as_str().unwrap(), p["alias"].as_str().unwrap())).collect();
...
fn assert_list_envelope(parsed: &serde_json::Value) -> &Vec<serde_json::Value> {
    ...
    assert_eq!(parsed["schema_version"].as_u64().unwrap(), 2, "{ctx}: schema_version should be 2");
...
fn assert_list_item(obj: &serde_json::Value, idx: usize) {
    ...
    assert_eq!(obj["schema_version"].as_u64().unwrap(), 2, "{ctx}: per-item schema_version should be 2");
...
fn contract_list_json_empty() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    let parsed: serde_json::Value = serde_json::from_str(&stdout).unwrap_or_else(|e| panic!("Invalid JSON from list: {e}\nOutput: {stdout}"));
...
fn contract_list_json_shape_codex() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "list", "--workspace", "/data/projects/backend"]).output().expect("list should run");
...
fn contract_list_json_shape_gemini() {
    let tmp = TempDir::new().unwrap();
    setup_gemini_fixture_custom(&tmp, "gmi_simple", Some("/data/projects/cross_agent_session_resumer"));

    let output = casr_cmd(&tmp).args(["--json", "list", "--workspace", "/data/projects/cross_agent_session_resumer"]).output().expect("list should run");
...
fn contract_list_json_messages_is_nonnegative() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "list", "--workspace", "/data/projects/myapp"]).output().unwrap();
...
fn assert_info_object(obj: &serde_json::Value) {
    let ctx = "info";
    assert_exact_keys(obj, &["schema_version", "session_id", "provider", "title", "native_name", "workspace", "messages", "started_at", "ended_at", "model_name", "source_path", "metadata", "workspace_name", "workspace_name_source"], ctx);
    assert_uint(&obj["schema_version"], "schema_version", ctx);
    assert_eq!(obj["schema_version"].as_u64().unwrap(), 2, "{ctx}: schema_version should be 2");
    assert_string(&obj["session_id"], "session_id", ctx);
    assert_string(&obj["provider"], "provider", ctx);
    assert_string_or_null(&obj["title"], "title", ctx);
    assert_string_or_null(&obj["native_name"], "native_name", ctx);
    ...
    assert!(obj["metadata"].is_object() || obj["metadata"].is_null(), "{ctx}: metadata should be object|null");
    assert_string_or_null(&obj["workspace_name"], "workspace_name", ctx);
    assert_string_or_null(&obj["workspace_name_source"], "workspace_name_source", ctx);
}
...
fn contract_info_json_shape_cc() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    let parsed: serde_json::Value = serde_json::from_str(&stdout).unwrap_or_else(|e| panic!("Invalid JSON from info: {e}\nOutput: {stdout}"));
...
fn contract_info_json_source_path_is_absolute() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "info", &session_id]).output().unwrap();

    let parsed: serde_json::Value = serde_json::from_str(&String::from_utf8_lossy(&output.stdout)).unwrap();

    let path = parsed["source_path"].as_str().unwrap();
    assert!(path.starts_with('/'), "source_path should be absolute, got: {path}");
}
...
fn assert_resume_success_object(obj: &serde_json::Value) {
    ...
    assert_exact_keys(obj, &["ok", "source_provider", "target_provider", "source_session_id", "target_session_id", "written_paths", "resume_command", "dry_run", "warnings"], ctx);
...
fn contract_resume_json_dry_run_cc_to_codex() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    let parsed: serde_json::Value = serde_json::from_str(&stdout).unwrap_or_else(|e| panic!("Invalid JSON from resume: {e}\nOutput: {stdout}"));
...
fn contract_resume_json_actual_write_cc_to_codex() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "resume", "cod", &session_id]).output().expect("resume should run");
    ...
    let paths = parsed["written_paths"].as_array().expect("written_paths should be array on actual write");
...
fn contract_resume_json_actual_write_cc_to_gemini() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "resume", "gmi", &session_id]).output().expect("resume should run");
...
fn contract_resume_json_warnings_are_strings() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "resume", "cod", &session_id, "--dry-run"]).output().unwrap();
...
fn contract_resume_json_codex_to_cc() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "resume", "cc", &session_id]).output().expect("resume should run");
...
fn contract_error_json_unknown_session() {
    let tmp = TempDir::new().unwrap();
    let output = casr_cmd(&tmp).args(["--json", "info", "nonexistent-session-id-12345"]).output().expect("info should run");
...
fn contract_error_json_unknown_provider() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "resume", "nonexistent", &session_id]).output().expect("resume should run");
    ...
    assert_error_envelope(&parsed);
    assert_eq!(parsed["error_type"].as_str().unwrap(), "UnknownProviderAlias");
}

#[test]
fn contract_error_json_unknown_resume_session() {
    let tmp = TempDir::new().unwrap();
    let output = casr_cmd(&tmp).args(["--json", "resume", "cod", "nonexistent-session-99999"]).output().expect("resume should run");
...
fn contract_error_json_message_is_nonempty() {
    let tmp = TempDir::new().unwrap();
    let output = casr_cmd(&tmp).args(["--json", "info", "nonexistent-session-id-12345"]).output().unwrap();
    ...
    assert!(msg.contains("nonexistent-session-id-12345"), "error message should reference the session id");
}

#[test]
fn contract_error_json_known_error_types() {
    // Verify all error types map to valid CasrError variant names.
    let known_types = ["SessionNotFound", "AmbiguousSessionId", "UnknownProviderAlias", "ProviderUnavailable", "SessionReadError", "SessionWriteError", "SessionConflict", "ValidationError", "VerifyFailed", "InternalError"];

    // Trigger SessionNotFound.
    let tmp = TempDir::new().unwrap();
    let output = casr_cmd(&tmp).args(["--json", "info", "no-such-session"]).output().unwrap();

    let stderr = String::from_utf8_lossy(&output.stderr);
    let parsed: serde_json::Value = serde_json::from_str(&stderr).unwrap();
    let error_type = parsed["error_type"].as_str().unwrap();
    assert!(known_types.contains(&error_type), "error_type '{error_type}' not in known types: {known_types:?}");
}
...
fn contract_success_json_on_stdout_not_stderr() {
    ...
    assert!(!output.stdout.is_empty(), "success JSON should be on stdout");
    // Stderr should be empty or contain only trace/debug logs (not JSON).
    let stderr = String::from_utf8_lossy(&output.stderr);
    if !stderr.is_empty() {
        assert!(serde_json::from_str::<serde_json::Value>(&stderr).is_err(), "stderr should not contain JSON on success");
    }
}
...
fn contract_error_json_on_stderr_not_stdout() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(serde_json::from_str::<serde_json::Value>(&stderr).is_ok(), "error JSON should be on stderr");
    // Stdout should be empty on error.
    assert!(output.stdout.is_empty(), "stdout should be empty on error, got: {}", String::from_utf8_lossy(&output.stdout));
}
...
fn contract_list_provider_field_matches_slug() {
    ...
    let valid_slugs = ["claude-code", "codex", "gemini", "antigravity", "cursor", "cline", "aider", "amp", "opencode", "chatgpt", "clawdbot", "vibe", "factory", "openclaw", "pi-agent", "kiro", "grok"];
    for item in items {
        let provider = item["provider"].as_str().unwrap();
        assert!(valid_slugs.contains(&provider), "list item provider '{provider}' not in known slugs");
    }
}
...
fn contract_resume_source_session_id_matches_input() {
    ...
    assert_eq!(parsed["source_session_id"].as_str().unwrap(), session_id, "source_session_id should match the input session ID");
}
...
fn contract_list_json_native_name_present() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "list", "--workspace", "/data/projects/named"]).output().expect("list should run");
    assert!(output.status.success());
    let parsed: serde_json::Value = serde_json::from_str(&String::from_utf8_lossy(&output.stdout)).unwrap();
    let items = assert_list_envelope(&parsed);
...
fn contract_list_json_native_name_absent_is_null() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "list", "--workspace", "/data/projects/plain"]).output().expect("list should run");
    assert!(output.status.success());
    let parsed: serde_json::Value = serde_json::from_str(&String::from_utf8_lossy(&output.stdout)).unwrap();
    let items = assert_list_envelope(&parsed);
    assert!(!items.is_empty(), "expected the session in the list");
    assert!(items[0]["native_name"].is_null(), "a session with no native name must be null: {:?}", items[0]["native_name"]);
}
...
fn contract_list_human_shows_name_column() {
    ...
    let output = casr_cmd(&tmp).args(["list", "--workspace", "/data/projects/named"]).output().expect("list should run");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Name"), "table should carry a Name column");
    assert!(stdout.contains("My Renamed Session"), "human list should render the native name: {stdout}");
}
...
fn contract_info_human_shows_name_line() {
    ...
    let output = casr_cmd(&tmp).args(["info", "rename-1"]).output().expect("info should run");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Name:") && stdout.contains("My Renamed Session"), "info should show the native Name line: {stdout}");
}
...
fn contract_info_peek_json_includes_ordered_tail() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "info", "rename-1", "--peek", "--peek-lines", "2"]).output().expect("info --peek should run");
    assert!(output.status.success());
    let parsed: serde_json::Value = serde_json::from_str(&String::from_utf8_lossy(&output.stdout)).unwrap();

    let tail = parsed["transcript_tail"].as_array().expect("transcript_tail should be an array with --peek");
...
fn contract_info_without_peek_has_no_tail() {
    ...
    let output = casr_cmd(&tmp).args(["--json", "info", "rename-1"]).output().expect("info should run");
    assert!(output.status.success());
    let parsed: serde_json::Value = serde_json::from_str(&String::from_utf8_lossy(&output.stdout)).unwrap();
    assert!(parsed.get("transcript_tail").is_none(), "transcript_tail must be omitted without --peek");
}
...
fn contract_info_human_peek_shows_tail_section() {
    ...
    let output = casr_cmd(&tmp).args(["info", "rename-1", "--peek", "--peek-lines", "3"]).output().expect("info --peek should run");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Transcript Tail"), "peek should append a Transcript Tail section: {stdout}");
