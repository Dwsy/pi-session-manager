...
impl Factory {
    ...
    fn extract_tool_calls(message_obj: Option<&serde_json::Value>, content_value: Option<&serde_json::Value>) -> Vec<ToolCall> {
        ...
        if let Some(serde_json::Value::Array(blocks)) = content_value {
            for block in blocks {
                ...
                let arguments = obj.get("input").or_else(|| obj.get("arguments")).or_else(|| obj.get("args")).cloned().unwrap_or(serde_json::Value::Null);
                calls.push(ToolCall {
                    id: obj.get("id").or_else(|| obj.get("call_id")).or_else(|| obj.get("tool_use_id")).and_then(|v| v.as_str()).map(String::from),
                    name: obj.get("name").or_else(|| obj.get("function").and_then(|v| v.get("name"))).and_then(|v| v.as_str()).unwrap_or("unknown").to_string(),
                    arguments,
                });
            }
        }

        if let Some(tool_calls) = message_obj.and_then(|m| m.get("toolCalls")).and_then(|v| v.as_array()) {
            for call in tool_calls {
                let Some(obj) = call.as_object() else {
                    continue;
                };
                calls.push(ToolCall {
                    id: obj.get("id").or_else(|| obj.get("call_id")).and_then(|v| v.as_str()).map(String::from),
                    name: obj.get("name").or_else(|| obj.get("function").and_then(|v| v.get("name"))).and_then(|v| v.as_str()).unwrap_or("unknown").to_string(),
                    arguments: obj.get("input").or_else(|| obj.get("arguments")).or_else(|| obj.get("args")).cloned().unwrap_or(serde_json::Value::Null),
                });
            }
        }
    ...
    fn extract_tool_results(message_obj: Option<&serde_json::Value>, content_value: Option<&serde_json::Value>) -> Vec<ToolResult> {
        ...
        if let Some(serde_json::Value::Array(blocks)) = content_value {
            for block in blocks {
                ...
                let result_content = obj.get("content").or_else(|| obj.get("output")).or_else(|| obj.get("result")).map(flatten_content).unwrap_or_default();
                let is_error = obj.get("is_error").and_then(|v| v.as_bool()).or_else(|| obj.get("status").and_then(|v| v.as_str()).map(|s| s == "error")).unwrap_or(false);
                results.push(ToolResult { call_id: obj.get("tool_use_id").or_else(|| obj.get("call_id")).or_else(|| obj.get("id")).and_then(|v| v.as_str()).map(String::from), content: result_content, is_error });
            }
        }

        if let Some(tool_results) = message_obj.and_then(|m| m.get("toolResults")).and_then(|v| v.as_array()) {
            for result in tool_results {
                let Some(obj) = result.as_object() else {
                    continue;
                };
                results.push(ToolResult {
                    call_id: obj.get("tool_use_id").or_else(|| obj.get("call_id")).or_else(|| obj.get("id")).and_then(|v| v.as_str()).map(String::from),
                    content: obj.get("content").or_else(|| obj.get("output")).or_else(|| obj.get("result")).map(flatten_content).unwrap_or_default(),
                    is_error: obj.get("is_error").and_then(|v| v.as_bool()).or_else(|| obj.get("status").and_then(|v| v.as_str()).map(|s| s == "error")).unwrap_or(false),
                });
            }
        }
...
impl Provider for Factory {
    ...
    fn owns_session(&self, session_id: &str) -> Option<PathBuf> {
        ...
        for entry in walkdir::WalkDir::new(&root).max_depth(3).into_iter().filter_map(Result::ok) {
            if entry.file_type().is_file() && entry.file_name().to_str().is_some_and(|n| n == target_name) {
                debug!(
                    provider = "factory",
    ...
    fn read_session(&self, path: &Path) -> anyhow::Result<CanonicalSession> {
        ...
        for line_result in reader.lines() {
            ...
            match entry_type {
                Some("session_start") => {
                    ...
                    workspace = val.get("cwd").and_then(|v| v.as_str()).map(PathBuf::from).or_else(|| parent_dir_name.and_then(Self::decode_workspace_slug));
                }
                Some("message") => {
                    ...
                    let role_str = val.get("message").and_then(|m| m.get("role")).and_then(|v| v.as_str()).unwrap_or("unknown");
                    ...
                    let author = message_obj.and_then(|m| m.get("model")).and_then(|v| v.as_str()).map(String::from);
        ...
        let title = title_from_header.or_else(|| messages.iter().find(|m| m.role == MessageRole::User).map(|m| truncate_title(&m.content, 100)));
        ...
        info!(session_id, messages = messages.len(), "Factory session parsed");

        Ok(CanonicalSession { session_id, provider_slug: "factory".to_string(), workspace, title, started_at, ended_at, messages, metadata, source_path: path.to_path_buf(), model_name: model_from_settings })
    }
    ...
    fn write_session(&self, session: &CanonicalSession, opts: &WriteOptions) -> anyhow::Result<WrittenSession> {
        ...
        let workspace_slug = session.workspace.as_ref().map(|p| Self::encode_workspace_slug(p)).unwrap_or_else(|| "-tmp".to_string());
        ...
        for msg in &session.messages {
            ...
            message_obj.insert("role".into(), serde_json::Value::String(role_str.to_string()));
            message_obj.insert("content".into(), serde_json::Value::String(msg.content.clone()));
            if let Some(ref author) = msg.author {
                message_obj.insert("model".into(), serde_json::Value::String(author.clone()));
            }

            let mut entry = serde_json::Map::new();
            entry.insert("type".into(), serde_json::Value::String("message".to_string()));
            if let Some(ts) = msg.timestamp {
                let dt = chrono::DateTime::from_timestamp_millis(ts).unwrap_or_else(chrono::Utc::now);
                entry.insert("timestamp".into(), serde_json::Value::String(dt.to_rfc3339()));
            }
...
mod tests {
    ...
    fn reader_session_id_from_header() {
        let session = read_factory("-test", "file-name", &[r#"{"type":"session_start","id":"header-id"}"#, r#"{"type":"message","message":{"role":"user","content":"test"}}"#]);
        assert_eq!(session.session_id, "header-id");
    }

    #[test]
    fn reader_session_id_fallback_to_filename() {
        let session = read_factory("-test", "fallback-name", &[r#"{"type":"message","message":{"role":"user","content":"test"}}"#]);
        assert_eq!(session.session_id, "fallback-name");
    }

    #[test]
    fn reader_workspace_from_cwd() {
        let session = read_factory("-test", "ws-test", &[r#"{"type":"session_start","cwd":"/data/projects/app"}"#, r#"{"type":"message","message":{"role":"user","content":"test"}}"#]);
        assert_eq!(session.workspace, Some(PathBuf::from("/data/projects/app")));
    }

    #[test]
    fn reader_workspace_fallback_to_slug() {
        let session = read_factory("-Users-alice-Dev-myproject", "ws-slug", &[r#"{"type":"session_start","id":"ws-slug"}"#, r#"{"type":"message","message":{"role":"user","content":"test"}}"#]);
        assert_eq!(session.workspace, Some(PathBuf::from("/Users/alice/Dev/myproject")));
    }

    #[test]
    fn reader_title_from_header() {
        let session = read_factory("-test", "title-h", &[r#"{"type":"session_start","title":"Header Title"}"#, r#"{"type":"message","message":{"role":"user","content":"user msg"}}"#]);
        assert_eq!(session.title.as_deref(), Some("Header Title"));
    }

    #[test]
    fn reader_title_fallback_to_user_message() {
        let session = read_factory("-test", "title-u", &[r#"{"type":"session_start"}"#, r#"{"type":"message","message":{"role":"user","content":"First user message"}}"#]);
        assert_eq!(session.title.as_deref(), Some("First user message"));
    }

    #[test]
    fn reader_skips_unknown_entry_types() {
        let session = read_factory("-test", "skip-types", &[r#"{"type":"todo_state","tasks":[]}"#, r#"{"type":"tool_result","name":"bash","output":"ok"}"#, r#"{"type":"message","message":{"role":"user","content":"Real message"}}"#]);
    ...
    fn reader_skips_empty_content() {
        let session = read_factory("-test", "empty-c", &[r#"{"type":"message","message":{"role":"user","content":"Valid"}}"#, r#"{"type":"message","message":{"role":"assistant","content":""}}"#, r#"{"type":"message","message":{"role":"assistant","content":"   "}}"#]);
        assert_eq!(session.messages.len(), 1);
    }

    #[test]
    fn reader_extracts_model_as_author() {
        let session = read_factory("-test", "model-a", &[r#"{"type":"message","message":{"role":"assistant","content":"Response","model":"claude-opus"}}"#]);
        assert_eq!(session.messages[0].author.as_deref(), Some("claude-opus"));
    }

    #[test]
    fn reader_handles_array_content() {
        let session = read_factory("-test", "arr-c", &[r#"{"type":"message","message":{"role":"assistant","content":[{"type":"text","text":"Part 1"},{"type":"text","text":"Part 2"}]}}"#]);
    ...
    fn reader_extracts_tool_calls_and_tool_results_from_content_blocks() {
        ...
        assert_eq!(session.messages[1].tool_results[0].call_id.as_deref(), Some("toolu_1"));
        assert_eq!(session.messages[1].tool_results[0].content, "ok");
    }

    #[test]
    fn reader_extracts_tool_calls_from_message_level_tool_calls() {
        let session = read_factory("-test", "tool-calls-array", &[r#"{"type":"message","message":{"role":"assistant","content":"Running","toolCalls":[{"id":"call_1","name":"Grep","args":{"pattern":"tool"}}]}}"#]);
        assert_eq!(session.messages.len(), 1);
        assert_eq!(session.messages[0].tool_calls.len(), 1);
        assert_eq!(session.messages[0].tool_calls[0].name, "Grep");
        assert_eq!(session.messages[0].tool_calls[0].id.as_deref(), Some("call_1"));
    }
    ...
    fn reader_loads_settings_file() {
        ...
        std::fs::write(&session_path, r#"{"type":"message","message":{"role":"user","content":"test"}}"#).unwrap();
    ...
    fn reader_metadata_has_source() {
        let session = read_factory("-test", "meta", &[r#"{"type":"message","message":{"role":"user","content":"test"}}"#]);
    ...
    fn decode_workspace_slug_basic() {
        assert_eq!(Factory::decode_workspace_slug("-Users-alice-Dev-myproject"), Some(PathBuf::from("/Users/alice/Dev/myproject")));
    }
    ...
    fn encode_workspace_slug_basic() {
        assert_eq!(Factory::encode_workspace_slug(Path::new("/Users/alice/Dev")), "-Users-alice-Dev");
    }
    ...
    fn writer_produces_valid_factory_jsonl() {
        ...
        let session = CanonicalSession {
            ...
            messages: vec![
                CanonicalMessage { idx: 0, role: MessageRole::User, content: "Fix it".to_string(), timestamp: Some(1_700_000_000_000), author: None, tool_calls: vec![], tool_results: vec![], extra: json!({}) },
                CanonicalMessage { idx: 1, role: MessageRole::Assistant, content: "Done.".to_string(), timestamp: Some(1_700_000_500_000), author: Some("claude-3".to_string()), tool_calls: vec![], tool_results: vec![], extra: json!({}) },
            ],
            metadata: json!({}),
    ...
    fn writer_resume_command() {
        let provider = Factory;
        assert_eq!(provider.resume_command("my-session"), "factory --resume my-session");
    }
