...
use crate::model::{CanonicalMessage, CanonicalSession, MessageRole, ToolCall, ToolResult, flatten_content, normalize_role, reindex_messages, truncate_title};
...
impl Cline {
    ...
    fn storage_roots() -> Vec<PathBuf> {
        ...
        host_roots.into_iter().map(|host| host.join("User").join("globalStorage").join(CLINE_EXTENSION_ID)).filter(|p| p.is_dir()).collect()
    }
    ...
    fn task_id_from_task_dir(task_dir: &Path) -> Option<String> {
        task_dir.file_name().map(|s| s.to_string_lossy().to_string())
    }
    ...
    fn read_task_history_item(storage_root: &Path, task_id: &str) -> Option<serde_json::Map<String, serde_json::Value>> {
    ...
    fn extract_tool_results(content: Option<&serde_json::Value>) -> Vec<ToolResult> {
        ...
        blocks
            ...
            .filter_map(|block| {
                ...
                let content_value = obj.get("content").or_else(|| obj.get("output")).unwrap_or(&serde_json::Value::Null);
                Some(ToolResult { call_id: obj.get("tool_use_id").and_then(|v| v.as_str()).map(String::from), content: flatten_content(content_value), is_error: obj.get("is_error").and_then(|v| v.as_bool()).unwrap_or(false) })
            })
            .collect()
    }
    ...
    fn pick_storage_root_for_write() -> anyhow::Result<PathBuf> {
        ...
        roots.into_iter().next().ok_or_else(|| anyhow::anyhow!("Cline storage not found. Set CLINE_HOME to the extension globalStorage directory."))
    }
    ...
    fn build_ui_messages(session: &CanonicalSession) -> Vec<serde_json::Value> {
        ...
        for msg in &session.messages {
            let (say, text) = match msg.role {
                ...
                MessageRole::Tool | MessageRole::System | MessageRole::Other(_) => ("info", msg.content.clone()),
            };
    ...
    fn update_task_history(storage_root: &Path, task_id: &str, session: &CanonicalSession, provider_slug: &str) -> anyhow::Result<Option<PathBuf>> {
        let history_path = Self::task_history_path(storage_root);
        ...
        let title = session.title.clone().or_else(|| session.messages.iter().find(|m| m.role == MessageRole::User).map(|m| truncate_title(&m.content, 100))).unwrap_or_else(|| "Untitled Task".to_string());

        let ts = session.started_at.unwrap_or_else(|| chrono::Utc::now().timestamp_millis());
        ...
        obj.insert("totalCost".into(), serde_json::Value::Number(serde_json::Number::from_f64(0.0).unwrap_or_else(|| 0.into())));
        if let Some(ws) = session.workspace.as_ref() {
            obj.insert("cwdOnTaskInitialization".into(), serde_json::Value::String(ws.display().to_string()));
        }
        ...
        let bytes = serde_json::to_vec_pretty(&serde_json::Value::Array(items)).context("failed to serialize taskHistory.json")?;
        ...
        let outcome = crate::pipeline::atomic_write(&history_path, &bytes, true, provider_slug).map_err(|e| anyhow::anyhow!("{e}"))?;
...
impl Provider for Cline {
    ...
    fn session_roots(&self) -> Vec<PathBuf> {
        Self::storage_roots().into_iter().map(|root| Self::tasks_root(&root)).collect()
    }
    ...
    fn read_session(&self, path: &Path) -> anyhow::Result<CanonicalSession> {
        ...
        if !matches!(file_name, FILE_API_HISTORY | FILE_UI_MESSAGES | FILE_UI_MESSAGES_OLD) {
            return Err(anyhow::anyhow!("unsupported Cline session path (expected task file): {}", path.display()));
        }

        let task_dir = Self::task_dir_from_api_path(path).ok_or_else(|| anyhow::anyhow!("not a Cline task path: {}", path.display()))?;
        let task_id = Self::task_id_from_task_dir(&task_dir).ok_or_else(|| anyhow::anyhow!("could not derive task id: {}", task_dir.display()))?;
        let storage_root = Self::find_storage_root_for_path(path).ok_or_else(|| anyhow::anyhow!("could not derive Cline storage root for {}", path.display()))?;
        ...
        let api_source_path = if file_name == FILE_API_HISTORY {
            ...
            return Err(anyhow::anyhow!("non-primary Cline task artifact (use {}): {}", FILE_API_HISTORY, path.display()));
        } else {
        ...
        if api_source_path.file_name().and_then(|s| s.to_str()).is_some_and(|n| n == FILE_API_HISTORY) {
            ...
            for item in items {
                ...
                let author = obj.get("modelInfo").and_then(|v| v.get("modelId")).and_then(|v| v.as_str()).filter(|s| !s.is_empty()).map(String::from);
                ...
                messages.push(CanonicalMessage { idx: 0, role, content, timestamp: None, author, tool_calls, tool_results, extra: serde_json::Value::Object(obj.clone()) });
            }
        } else {
            ...
            for item in items {
                ...
                messages.push(CanonicalMessage { idx: 0, role, content: text.to_string(), timestamp: ts, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Object(obj.clone()) });
            }
        }
        ...
        let workspace = history_item.as_ref().and_then(|h| h.get("cwdOnTaskInitialization")).and_then(|v| v.as_str()).map(PathBuf::from);
        let started_at = history_item.as_ref().and_then(|h| h.get("ts")).and_then(|v| v.as_i64());

        let title = history_item.as_ref().and_then(|h| h.get("task")).and_then(|v| v.as_str()).map(|s| truncate_title(s, 100)).or_else(|| messages.iter().find(|m| m.role == MessageRole::User).map(|m| truncate_title(&m.content, 100)));

        let model_name = model_counts.into_iter().max_by_key(|(_, count)| *count).map(|(name, _)| name).or_else(|| history_item.as_ref().and_then(|h| h.get("modelId")).and_then(|v| v.as_str()).map(String::from));

        let mut metadata = serde_json::Map::new();
        metadata.insert("source".into(), serde_json::Value::String("cline".to_string()));
        ...
        Ok(CanonicalSession { session_id: task_id, provider_slug: "cline".to_string(), workspace, title, started_at, ended_at: started_at, messages, metadata: serde_json::Value::Object(metadata), source_path: api_source_path, model_name })
    }
    ...
    fn write_session(&self, session: &CanonicalSession, opts: &WriteOptions) -> anyhow::Result<WrittenSession> {
        ...
        std::fs::create_dir_all(&task_dir).with_context(|| format!("failed to create {}", task_dir.display()))?;

        // 1) api_conversation_history.json
        let api_history = Self::build_api_history(session);
        let api_bytes = serde_json::to_vec(&api_history).context("failed to serialize api history")?;
        let api_path = task_dir.join(FILE_API_HISTORY);
        let _ = crate::pipeline::atomic_write(&api_path, &api_bytes, opts.force, self.slug())?;

        // 2) ui_messages.json
        let ui_messages = Self::build_ui_messages(session);
        let ui_bytes = serde_json::to_vec(&ui_messages).context("failed to serialize ui messages")?;
        ...
        let _ = crate::pipeline::atomic_write(&metadata_path, &metadata_bytes, opts.force, self.slug())?;

        // 4) state/taskHistory.json (best-effort, but needed for Cline to list tasks)
        let backup_path = Self::update_task_history(&storage_root, &target_task_id, session, self.slug())?;
...
mod tests {
    ...
    fn write_api_session(task_id: &str, api_entries: &[serde_json::Value], task_history: Option<&[serde_json::Value]>) -> (tempfile::TempDir, std::path::PathBuf) {
    ...
    fn reader_api_basic_exchange() {
        let entries = vec![make_api_user("Hello world"), make_api_assistant("Hi there!")];
    ...
    fn reader_api_tool_use_blocks() {
        ...
        assert_eq!(assistant.tool_calls[0].arguments["path"].as_str(), Some("src/main.rs"));
    }
    ...
    fn reader_api_tool_result_blocks() {
        ...
        let session = Cline.read_session(&api_path).expect("read_session");
        let tool_msg = session.messages.iter().find(|m| !m.tool_results.is_empty()).expect("should have a message with tool_results");
        ...
        assert_eq!(tool_msg.tool_results[0].call_id.as_deref(), Some("tool-abc"));
    ...
    fn reader_api_model_info_extraction() {
        let entries = vec![make_api_user("Hello"), make_api_assistant_with_model("Hi", "claude-3.5-sonnet"), make_api_user("Another"), make_api_assistant_with_model("Reply", "claude-3.5-sonnet")];
        ...
        assert_eq!(session.messages[1].author.as_deref(), Some("claude-3.5-sonnet"));
    }

    #[test]
    fn reader_api_skips_empty_content() {
        let entries = vec![make_api_user("Hello"), json!({"role": "assistant", "content": []}), json!({"role": "assistant", "content": [{"type": "text", "text": "  "}]}), make_api_assistant("Real answer")];
    ...
    fn reader_api_title_from_task_history() {
        ...
        let entries = vec![make_api_user("Fix the auth bug"), make_api_assistant("On it!")];
    ...
    fn reader_api_title_fallback() {
        ...
        let entries = vec![make_api_user("Implement dark mode support"), make_api_assistant("Sure!")];
        let (_root, api_path) = write_api_session("1700000000007", &entries, None);

        let session = Cline.read_session(&api_path).expect("read_session");
        assert_eq!(session.title.as_deref(), Some("Implement dark mode support"));
    }
    ...
    fn reader_api_workspace_from_history() {
        ...
        let session = Cline.read_session(&api_path).expect("read_session");
        assert_eq!(session.workspace, Some(PathBuf::from("/data/projects/my_app")));
    }
    ...
    fn reader_api_rejects_non_primary() {
        ...
        let ui_entries = vec![json!({"type": "say", "say": "task", "text": "Hello", "ts": 1700000000010_i64})];
        std::fs::write(&ui_path, serde_json::to_vec(&ui_entries).unwrap()).unwrap();

        let err = Cline.read_session(&ui_path).unwrap_err();
        let msg = format!("{err}");
        assert!(msg.contains("non-primary"), "expected 'non-primary' error, got: {msg}");
    }
    ...
    fn build_api_history_structure() {
        let session = make_canonical_session(vec![
            CanonicalMessage { idx: 0, role: MessageRole::User, content: "Hello".to_string(), timestamp: None, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage {
                idx: 1,
                ...
                timestamp: None,
                author: None,
                tool_calls: vec![ToolCall { id: Some("tc-1".to_string()), name: "Read".to_string(), arguments: json!({"path": "main.rs"}) }],
                tool_results: vec![],
        ...
        let blocks1 = api[1]["content"].as_array().expect("assistant content blocks");
    ...
    fn build_api_history_tool_results_in_user_message() {
        let session = make_canonical_session(vec![CanonicalMessage {
            ...
            author: None,
            tool_calls: vec![],
            tool_results: vec![ToolResult { call_id: Some("tc-1".to_string()), content: "file contents".to_string(), is_error: false }],
            extra: serde_json::Value::Null,
        }]);
        ...
        assert!(blocks.iter().any(|b| b["type"].as_str() == Some("tool_result")), "expected tool_result block");
        let tr_block = blocks.iter().find(|b| b["type"].as_str() == Some("tool_result")).expect("tool_result block");
    ...
    fn build_ui_messages_first_user_is_task() {
        let session = make_canonical_session(vec![
            CanonicalMessage { idx: 0, role: MessageRole::User, content: "Fix the bug".to_string(), timestamp: Some(1_700_000_000_000), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 1, role: MessageRole::Assistant, content: "On it".to_string(), timestamp: Some(1_700_000_000_001), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
        ]);
    ...
    fn build_ui_messages_subsequent_user_is_feedback() {
        let session = make_canonical_session(vec![
            CanonicalMessage { idx: 0, role: MessageRole::User, content: "Initial task".to_string(), timestamp: Some(1_700_000_000_000), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 1, role: MessageRole::Assistant, content: "OK".to_string(), timestamp: Some(1_700_000_000_001), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 2, role: MessageRole::User, content: "Follow up".to_string(), timestamp: Some(1_700_000_000_002), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
        ]);
    ...
    fn build_ui_messages_skips_empty_content() {
        let session = make_canonical_session(vec![
            CanonicalMessage { idx: 0, role: MessageRole::User, content: "Hello".to_string(), timestamp: Some(1_700_000_000_000), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 1, role: MessageRole::Assistant, content: "  ".to_string(), timestamp: Some(1_700_000_000_001), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 2, role: MessageRole::Assistant, content: "Real reply".to_string(), timestamp: Some(1_700_000_000_002), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
        ]);
    ...
    fn writer_roundtrip_via_build_and_read() {
        let session = make_canonical_session(vec![
            CanonicalMessage { idx: 0, role: MessageRole::User, content: "Hello roundtrip".to_string(), timestamp: Some(1_700_000_000_000), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage {
                idx: 1,
                role: MessageRole::Assistant,
                ...
                author: None,
                tool_calls: vec![ToolCall { id: Some("tc-rt".to_string()), name: "Read".to_string(), arguments: json!({"path": "lib.rs"}) }],
                tool_results: vec![],
                extra: serde_json::Value::Null,
            },
        ]);
        ...
        for (orig, rb) in session.messages.iter().zip(readback.messages.iter()) {
            ...
            assert!(rb.content.starts_with(&orig.content), "readback content '{}' should start with original '{}'", rb.content, orig.content);
        }
    ...
    fn writer_api_history_role_assignments() {
        let session = make_canonical_session(vec![
            CanonicalMessage { idx: 0, role: MessageRole::User, content: "Hello".to_string(), timestamp: None, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 1, role: MessageRole::Assistant, content: "Hi".to_string(), timestamp: None, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 2, role: MessageRole::System, content: "System msg".to_string(), timestamp: None, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 3, role: MessageRole::Tool, content: "Tool output".to_string(), timestamp: None, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
        ]);
    ...
    fn writer_ui_messages_format() {
        let session = make_canonical_session(vec![
            CanonicalMessage { idx: 0, role: MessageRole::User, content: "Task text".to_string(), timestamp: Some(1_700_000_000_000), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 1, role: MessageRole::Assistant, content: "Reply".to_string(), timestamp: Some(1_700_000_000_001), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
        ]);
    ...
    fn writer_ui_messages_other_roles() {
        let session = make_canonical_session(vec![
            CanonicalMessage { idx: 0, role: MessageRole::User, content: "Hello".to_string(), timestamp: Some(1_700_000_000_000), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
            CanonicalMessage { idx: 1, role: MessageRole::System, content: "System note".to_string(), timestamp: Some(1_700_000_000_001), author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null },
        ]);
    ...
    fn writer_updates_task_history() {
        ...
        let session = make_canonical_session(vec![CanonicalMessage { idx: 0, role: MessageRole::User, content: "My task".to_string(), timestamp: None, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null }]);

        Cline::update_task_history(root.path(), "1700000099999", &session, "cline").expect("update_task_history");

        let history_path = state_dir.join(FILE_TASK_HISTORY);
        assert!(history_path.exists());
        let content: Vec<serde_json::Value> = serde_json::from_str(&std::fs::read_to_string(&history_path).unwrap()).unwrap();
    ...
    fn writer_task_history_sorted_newest_first() {
        ...
        let mut session = make_canonical_session(vec![CanonicalMessage { idx: 0, role: MessageRole::User, content: "New task".to_string(), timestamp: None, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null }]);
        session.started_at = Some(1_700_000_000_000);

        Cline::update_task_history(root.path(), "1700000000000", &session, "cline").expect("update_task_history");
    ...
    fn writer_task_history_deduplicates() {
        ...
        let session = make_canonical_session(vec![CanonicalMessage { idx: 0, role: MessageRole::User, content: "New version".to_string(), timestamp: None, author: None, tool_calls: vec![], tool_results: vec![], extra: serde_json::Value::Null }]);
    ...
    fn write_ui_session(task_id: &str, ui_entries: &[serde_json::Value], task_history: Option<&[serde_json::Value]>) -> (tempfile::TempDir, std::path::PathBuf) {
        let root = tempfile::tempdir().expect("tmpdir");
        let task_dir = root.path().join("tasks").join(task_id);
        std::fs::create_dir_all(&task_dir).expect("create task dir");
    ...
    fn reader_ui_basic_exchange() {
        let entries = vec![json!({"type": "say", "say": "task", "text": "Fix the bug", "ts": 1_700_000_000_000_i64}), json!({"type": "say", "say": "text", "text": "Working on it", "ts": 1_700_000_000_001_i64})];
    ...
    fn reader_ui_task_say_as_user() {
        let entries = vec![json!({"type": "say", "say": "task", "text": "Initial task text", "ts": 1_700_000_000_000_i64}), json!({"type": "say", "say": "text", "text": "OK", "ts": 1_700_000_000_001_i64})];
    ...
    fn reader_ui_user_feedback_as_user() {
        let entries = vec![json!({"type": "say", "say": "task", "text": "Start task", "ts": 1_700_000_000_000_i64}), json!({"type": "say", "say": "text", "text": "OK", "ts": 1_700_000_000_001_i64}), json!({"type": "say", "say": "user_feedback", "text": "Try again", "ts": 1_700_000_000_002_i64})];
    ...
    fn reader_ui_user_feedback_diff_as_user() {
        let entries = vec![json!({"type": "say", "say": "task", "text": "Start", "ts": 1_700_000_000_000_i64}), json!({"type": "say", "say": "user_feedback_diff", "text": "Diff feedback", "ts": 1_700_000_000_001_i64})];
    ...
    fn reader_ui_text_say_as_assistant() {
        let entries =
            vec![json!({"type": "say", "say": "task", "text": "Hello", "ts": 1_700_000_000_000_i64}), json!({"type": "say", "say": "text", "text": "Response text", "ts": 1_700_000_000_001_i64}), json!({"type": "say", "say": "completion_result", "text": "Done", "ts": 1_700_000_000_002_i64})];
    ...
    fn reader_ui_skips_non_say_types() {
        let entries = vec![json!({"type": "say", "say": "task", "text": "Hello", "ts": 1_700_000_000_000_i64}), json!({"type": "ask", "ask": "tool", "text": "Allow?", "ts": 1_700_000_000_001_i64}), json!({"type": "say", "say": "text", "text": "OK", "ts": 1_700_000_000_002_i64})];
    ...
    fn reader_ui_timestamps_parsed() {
        let entries = vec![json!({"type": "say", "say": "task", "text": "Hello", "ts": 1_700_000_000_000_i64}), json!({"type": "say", "say": "text", "text": "Hi", "ts": 1_700_000_000_500_i64})];
    ...
    fn reader_ui_fallback_when_no_api_file() {
        ...
        let entries = vec![json!({"type": "say", "say": "task", "text": "Task text", "ts": 1_700_000_000_000_i64}), json!({"type": "say", "say": "text", "text": "Response", "ts": 1_700_000_000_001_i64})];
    ...
    fn reader_ui_old_claude_messages_filename() {
        ...
        let entries = vec![json!({"type": "say", "say": "task", "text": "Old format", "ts": 1_700_000_000_000_i64}), json!({"type": "say", "say": "text", "text": "Reply", "ts": 1_700_000_000_001_i64})];
    ...
    fn find_storage_root_for_path_extracts_root() {
        let path = PathBuf::from("/home/user/.config/Code/User/globalStorage/saoudrizwan.claude-dev/tasks/123/api_conversation_history.json");
        let root = Cline::find_storage_root_for_path(&path);
        assert_eq!(root, Some(PathBuf::from("/home/user/.config/Code/User/globalStorage/saoudrizwan.claude-dev")));
    }
    ...
    fn read_task_history_item_found() {
        ...
        let history = vec![json!({"id": "111", "ts": 111, "task": "Task A"}), json!({"id": "222", "ts": 222, "task": "Task B"})];
        std::fs::write(state_dir.join(FILE_TASK_HISTORY), serde_json::to_vec(&history).unwrap()).unwrap();
    ...
    fn task_dir_from_api_path_valid() {
        ...
        assert_eq!(task_dir, Some(PathBuf::from("/home/user/.cline/tasks/123456")));
    }
    ...
    fn task_id_from_task_dir_extracts_id() {
        ...
        assert_eq!(Cline::task_id_from_task_dir(&path), Some("9876543210".to_string()));
    }
