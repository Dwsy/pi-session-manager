...
export interface PluginRecordSearchHit extends DbPluginRecord {
  score: number
}

export interface PluginRecordSearchResult {
  hits: PluginRecordSearchHit[]
  totalHits?: number
  hasMore?: boolean
}
...
export interface PsmRecordsClient {
  search(params: PluginRecordSearchParams): Promise<{ hits: PluginRecord[]; totalHits?: number; hasMore?: boolean }>
  listForScope(params: PluginRecordListParams): Promise<PluginRecord[]>
  upsert(params: PluginRecordUpsertParams): Promise<PluginRecord>
}
...
export interface PsmSearchClient {
  pluginRecords(params: PluginRecordSearchParams): Promise<{ hits: PluginRecord[]; totalHits?: number; hasMore?: boolean }>
}
