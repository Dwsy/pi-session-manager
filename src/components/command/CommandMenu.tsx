...
function formatResultTime(result: SearchPluginResult): string | null {
  ...
  const raw =
    meta?.timestamp || meta?.session?.modified || meta?.session?.created;
  if (!raw) return null;
...
export default function CommandMenu({
  ...
  return (
    <div className="w-full h-full min-h-0 flex flex-col overflow-hidden bg-background">
      ...
      {(!!normalizedQuery.trim() || isLabelsBrowseMode) &&
        !isSearching &&
        !searchError && (
        ...
        <div className="w-[400px] flex-shrink-0 border-r border-border/70 flex flex-col h-full overflow-hidden bg-surface/[0.22]">
          ...
          >
            ...
            {!isSearching &&
              !searchError &&
              !normalizedQuery.trim() &&
              !isLabelsBrowseMode && <CommandHints />}
            {!isSearching &&
              !searchError &&
              Object.entries(groupedResults).map(
                ([pluginId, pluginResults]) => {
                  if (
                    activeTab !== "all" &&
                    activeTab !==
                      TABS.find((tab) => tab.pluginId === pluginId)?.id
                  )
                    return null;
                      ...
                      <div className="space-y-2">
                        {pluginResults.map((result) => {
                          ...
                          const isMessageResult =
                            result.pluginId === "message-search";
                          const isLabelMatch =
                            ...
                            >
                              <div className="px-4 py-3.5">
                                <div className="flex items-start gap-3">
                                  {result.icon && (
                                    <div
                                      className={`mt-0.5 flex h-8 w-8 items-center justify-center rounded-lg border flex-shrink-0 ${
                                        isSelected
                                    ...
                                    snippetLines.length > 0 ? (
                                      <>
                                        ...
                                        <div className="mt-2 flex items-start justify-between gap-3">
                                          ...
                                          <div className="flex shrink-0 items-center gap-1.5 pt-0.5">
                                            ...
                                            {roleBadge && (
                                              <span
                                                className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${roleBadge.className}`}
                                              >
                                                {roleBadge.label}
                                              </span>
                                            )}
                                      ...
                                      <>
                                        <div className="flex items-start justify-between gap-3">
                                          <div className="min-w-0 flex-1">
                                            <div
                                              className={`truncate text-[14px] leading-5 ${isSelected ? "font-semibold text-foreground" : "font-medium text-foreground/92"}`}
                                            >
                                              {result.title}
                                            </div>
                                            {metaLine && (
