          ...
          {PRESET_COLORS.map((c, i) => {
            ...
            return (
              <button
                ...
                className={`absolute h-3.5 w-3.5 rounded-full ${getColorClass(c.id)} ${value === c.id ? 'ring-2 ring-info ring-offset-1 ring-offset-background' : ''}`}
      ...
      <div className="flex items-center gap-2 px-3 py-2 group">
        ...
        {editingId === tag.id ? (
          ...
          <>
            ...
            <button onClick={() => onCreateChild(tag.id)} className="p-1 text-muted-foreground/40 hover:text-info opacity-0 group-hover:opacity-100 motion-color motion-opacity focus-ring" title={t('components.tagManager.addChild')}>
              <Plus className="h-3 w-3" />
            </button>
            <button onClick={() => onToggleRules(tag.id)} className={`p-1 rounded motion-color motion-opacity focus-ring ${rulesExpanded ? 'text-info' : 'text-muted-foreground/40 hover:text-muted-foreground'} ${rules.length > 0 ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} title={t('tags.autoRules.title')}>
              <Zap className="h-3.5 w-3.5" />
            </button>
            <button onClick={() => onDelete(tag.id)} className="p-1 text-muted-foreground/40 hover:text-red-500 opacity-0 group-hover:opacity-100 motion-color motion-opacity focus-ring" title={t('tags.remove')}>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <button onClick={handleCreate} disabled={!newName.trim()} className="flex items-center gap-1.5 px-3 py-2 bg-info hover:bg-info/80 text-white text-sm rounded-lg motion-color focus-ring disabled:opacity-40">
